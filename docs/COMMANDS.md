...
- `image-optimization` — image-delivery scanner: AVIF/WebP format, responsive srcset/sizes, width/height (CLS), below-fold lazy, framework image-component adoption, oversized sources. (Owns format/dimensions/loading; LCP priority stays with `lcp-audit`.)
- `font-optimization` — web-font scanner: font-display (FOIT), critical-font preload, self-host remote Google Fonts, size-adjust fallback (swap-CLS), unicode-range subset, variable-font, woff2-first.
...
| Command                | Purpose                                                                          |
|------------------------|----------------------------------------------------------------------------------|
| `/design-review`       | Read-only audit: cite-or-halt findings on UX, design-system, a11y, and **composite-surface completeness** (a data-table / dashboard graded against the built-in table-stakes catalog — toolbar/search/filter/bulk/export/sticky-header, metric-tile+widget hierarchy — via `ui-design-sweep § normalize-surface`, scale-gated). |
| `/enhance-ui <description>` | **Single-area enhancement (DRY-aware)**. Phase 1.5 picks scope tier: **token** / **wrapper-variant** / **wrapper-extract** / **leaf-local** so the same button is not styled twice on two pages. Cleanup includes `duplicated-surface-styles` for `frontend-*`. Intent-gates "redesign / rethink / re-theme / new visual language" → `/redesign`·`/art-direct` (it preserves structure + language, never introduces a new one). Flags: `--scope`, `--auto-extract`, `--dry-detect`. Then `design-iterate` (`$SCOPE_TIER`, consumer-route screenshots) → re-enforce; unrendered consumers report `SKIPPED`, run ends with a `Not validated / Risks / Revert` footer. |
| `/design-first <scope>` | **The design-first front door** (v1.28+) — one command for "I want a design to look at", for anyone who does not already hold the twelve-command ui-ux map. Writes **nothing**: not code, not artboards, not tokens. Its whole substance is one question and one test, which look alike and are opposite in kind. The **question** (`$SOURCE` — does this surface stay inside our design system, or leave it) is a *permission* only the product owner holds; it is always asked, and inferring it for an in-repo surface HALTs. The **test** (is the fault this page's composition or the app's visual language) is a *diagnosis* earned from a render and a token source; it is always run and never asked. Six routing rows, every one ending on a canvas: composition → `/redesign --canvas`, language → `/art-direct --canvas`, no code yet → `design-canvas`, an external reference → `/clone-design`, "show me what's out there" → `design-system-catalog.md`, invent it → `/art-direct --reimagine --canvas`. Deliberately **not** named `/design` — Claude Code ships its own bundled `design` skill. Flags: `--source=`, `--no-canvas`, `--plan`. |
| `/redesign <description-or-path>` | **From-scratch page/flow rework** (v1.3+) — rethinks layout + IA + UX (NOT a restyle), rebuilds inside the app's existing design system (same tokens / components / spacing-type scale / locale + RTL). **Method-driven for design quality**: an 11-lens best-practice rubric (IA / hierarchy / rhythm / cognitive-load / states / consistency / a11y-by-design / mobile-first / locale-RTL / motion / micro-copy) that the run **diagnoses** the current page against, **designs** the proposal to satisfy, **self-critiques** before the gate, and **scores** the rendered result against (must measurably beat the diagnosis). **Mandatory approval gate**: presents a structured proposal (diagnosis → screen's job → direction → layout/IA → component map → state inventory → responsive → a11y/RTL → parity map → risks; driven by `ux-reviewer`) and writes no code until the user approves. Composes `design-system-architect` (extraction + conformance), `ui-design-sweep` closure verbs, optional post-approval `design-iterate` variants. Renders the rebuilt surface (Playwright) to earn its RTL / a11y / responsive checkmarks. Sibling to `/enhance-ui` + `/polish` (which preserve structure); routes "new visual identity" → `/art-direct` (upstream). Flags: `--direction`, `--yes` (unattended variant step — does NOT skip the approval gate), `--max-refine=<n>`, `--plan`. Frontend / mobile only. **Canvas gate (v1.27+)**: `--canvas` draws the Phase-4 proposal as artboards (via `design-canvas`) so the gate is answered against a picture of the states rather than prose; `--from-canvas=<path>` takes an already-approved canvas as the spec and skips re-deriving the layout — its Phase 4.5 still checks provenance (an `independent` canvas is refused, not adapted), scope match, and encodability against the system as it stands now, and still owes the diagnosis and parity map a picture cannot state. |
| `/art-direct <scope>` | **Set + INVENT the visual direction** (v1.5+) — the one command UPSTREAM of `/redesign`: it DECIDES and invents the visual language (concept, original layouts/shapes/type/colour/motion, signature moments) from the product's GOALS, instead of enforcing or refining the existing one. Driven by the `creative-director` agent. **Two modes**: `--evolve` (default — push the existing language further, on-brand) / `--reimagine` (greenfield from goals; existing system is input, not constraint). Diagnoses the current design with a cited point-of-view (15-label redline vocabulary), generates **three mechanically-distinct directions** (logo-swap + color-ramp-swap + ≥2-structural-axes divergence checks), renders candidates (`design-iterate`) + scores them on a 9-lens Direction rubric, then **ONE approval gate** — and on approval it **BUILDS** the chosen direction, auto-running `design-system-architect` (codify tokens) → `/redesign` (rebuild pages) → `/polish` (finish), bounded by `<scope>`. `--yes` skips the gate (design → build, no stop); `--plan` stops at the design (no build). Originality is the ceiling; the usability floor (computed-AA / 320px / RTL / reduced-motion / perf) is delegated to `ux-reviewer` and never re-audited. Flags: `--evolve`, `--reimagine`, `--yes`, `--direction`, `--surfaces=<n>`, `--render`/`--no-render`, `--plan`. Frontend / mobile only. **Canvas gate (v1.27+)**: `--canvas` shows the three directions as low-fi artboards instead of three paragraphs — deliberately wireframe, because structure is what is being chosen and a hi-fi render invites picking on paint, the one axis the divergence check excludes. |
| `/add-theme-variant <name>` | **ADD a new theme to a multi-theme app** (v1.17+) — creates a NEW `themes/<name>/` slot ONLY; **never** edits an existing theme or the shared (cross-theme) layer. Four gated aspects: **Additive** (`git diff` proves only new-slot paths changed), **Architecture** (slot parallels the default, resolution wired — glob/whitelist/resolver detected — SSR-safe, RTL-correct), **Parity** (item-by-item manifest diff vs the default theme: components + icons + states + layouts + **framework-library controls + charts**; counts must match; a miss falls back to the default theme = invisible bug), **Performance** (the project's perf conventions + a perf-check gate on the new theme's key routes), plus a **Visual** gate that renders the new theme and runs an **adversarial per-component audit** (separate critic, each component below-bar until justified from the screenshot) — framework-library controls (the filter/control bar) need explicit `:deep()` overrides and charts need a config re-theme; neither inherits the token layer, so a default-styled control/chart is a Parity FAILURE, not `present ✓` (verified from the render; shared-only cases surface as an architecture LIMIT, never a shared-layer edit). Design modes — **default / `--skin` = RE-SKIN** (mirror the base's layout + components, retune tokens; same look-direction, new palette — will NOT look genuinely new) vs **`--reimagine` = the only NEW-LOOK mode** (routes to `/art-direct` to invent a language AND re-composes each surface via `/redesign` — new layout, added section types; inherits the **beats-the-old + re-composition + named-loud-move HARD gates**, so a variant that renders ~identical to the base HALTs instead of shipping). Parity is FEATURE parity (no base capability dropped), NOT layout parity — reimagine may re-compose + add sections. The Frame step recommends `--reimagine` when the ask is a new/bold look but the flag wasn't passed. Executed by `theme-specialist`. **Multi-theme architectures only** — single-theme projects HALT → `/art-direct` / `/redesign`. Frontend / mobile only. |
| `/clone-design <url-or-image> [<out-dir>]` | **Clone an EXTERNAL design reference** (v1.18+) — a live URL or a screenshot → a self-contained folder of framework-neutral HTML/CSS: every page **template** + a reusable **section library**, styled to a design-token system **extracted** from the reference (color roles clustered from computed colors, detected type scale, spacing base unit, radii/elevation/breakpoints), then **verified** by rendering each built page and perceptually diffing it against the reference until it clears a fidelity bar. The one ui-ux command whose source of truth is **outside the repo** (fidelity measured from pixels, not taste) and the one that runs with **no existing project** (Stage 1 is stack-agnostic — plain HTML/CSS into a folder, no framework/idioms required). Ingest: URL → Playwright (measured signal) / image → vision (inferred). **Two stages** — Stage 1 (capture → build → verify) is the whole command; Stage 2 adoption is a delegated `--adopt` follow-through (`=tokens` → `/add-theme-variant` · `=pages` → `/redesign` · `=theme:<platform>` → emit platform templates · `=project` → scaffold). Four gates: **Capture** (a blocked bot-wall/login/blank-shell HALTs), **Fidelity** (HARD — per-page pixel diff + color ΔE/type/spacing/layout, loops to `--max-refine`, below-bar = INCOMPLETE not faked), **Self-contained** (pages open offline), **Brand-safety** (HARD — reproduce the design *language*, placeholder the brand *identity*: logo/name/copy/photography → placeholders; never a deceptive counterfeit). Flags: `--pages=`, `--all-routes`, `--sections-only`, `--fidelity=`, `--max-refine=`, `--adopt=`, `--plan`. Sibling to `/art-direct` (which invents a language with no external reference). |
| `/grab-site <url> [<out-dir>]` | **Faithfully MIRROR a live website** (v1.19+) — real HTML + real CSS + real images + real fonts, one page per **template family** (home/collection/product/cart/search/page/article, auto-discovered from the homepage or an explicit `--pages` list), every remote reference rewritten to a local path so the folder **opens offline and looks like the original**. This is a **GRAB** (reproduce the real design as-is) — the deliberate opposite of `/clone-design`, which extracts a brand-neutral design *system* and *placeholders* the brand (that placeholdering is why pointing `/clone-design` at a real storefront yields a grey wireframe, not the site). Executes a **bundled stdlib-Python mirror script** (no pip/wget/httrack — only `python3`): fetch with a real UA → auto-discover template pages → download each page's CSS (recursing `@import` + `url()` for fonts/bg) + `img` src/data-src/srcset + `background-image` → rewrite all refs local + copy lazy `data-src` into `src` so images show without JS. Four gates: **Capture** (non-200/bot-wall HALTs), **Asset-localization** (real CSS + `<img src>` images must resolve local), **Offline-open** (pages render standalone; residual remote variants reported), **Template-coverage** (one per discovered family). **Honest limits**: JS interactions (sliders/cart/live-search) aren't live — a faithful *visual* copy; and it carries the source's real content, so swap your own brand/products in before shipping (not for passing a site off as its owner). Stack-agnostic + project-optional. Flags: `--pages=`, `--max-assets=`, `--plan`. Sibling to `/clone-design`. |
| `/ui-audit [<scope>]` | **The design team in one command** (v1.30+) — the visual sibling of `/audit`, and the pack's only fully autonomous run. Scans every route from a **real browser**, cross-axis ranks every finding by `user-impact × blast-radius × fix-cost`, **decides the design direction itself**, then executes four tiers to completion: **V0** foundation (tokens + shared wrappers — one fix, every consumer), **V1** correctness of experience (contrast, focus, tap targets, empty/loading/error states), **V2** composition (`/redesign` per surface that failed the language-or-composition test), **V3** finish (hierarchy, type scale, rhythm, density, iconography, motion). It adds no detector of its own — it drives `/ui-crawl`, `/ui-sweep`'s 8 detectors, `creative-director`'s 9-lens Direction rubric and `ui-design-sweep`'s closed 19 verbs. **The autonomy is the feature**: every gate its siblings hold for a human is resolved by a named mechanism (the variant pick becomes the rubric; the `/redesign` approval becomes one revertable commit per surface; the `$SOURCE` question is always `system`). **Five HALTs, and only five** — a blocked render, no token source, a dirty tree or red CI, a fix that re-detects after being applied, or `--reimagine` on a multi-theme app. Never scores a route it did not render and never reports a fix it did not re-render. Writes `ai/ui-audit/{plan,progress}.md` plus a before/after HTML report. Flags: `--plan`, `--scope=`, `--tier=`, `--first-run`, `--re-scan`, `--reimagine`, `--max-parallel=`. Frontend / mobile only. |
| `/ui-sweep [<phase>]`  | **Project-wide UI/UX specialist sweep**. Goes beyond align — runs 8 UI/UX-specific deep detectors (visual hierarchy, component utilization %, token coverage %, cross-surface consistency, ui-state coverage, responsive matrix, design-language coherence, visual baseline + drift). Phases by user flow (auth / checkout / dashboard / etc.), not by class. Outputs an HTML visual report with screenshots + metrics. Auth-gated routes require an authenticated session; a blocked login-wall render is marked BLOCKED (never scored). Flags: `--plan` (read-only fix plan, stop before FIX), `--baseline-only`, `--first-run`, `--with-iterate`, `--report-only`, `--allow-dirty`. Frontend stacks only. |
...
| `/ui-crawl-fix [<class>]` | **Wrapper-level auto-fixer for `/ui-crawl` findings** (v1.2+). Patches at shared wrappers (`FormField`, `CrudActions`, `TableActions`, `BaseModal`, etc.) so one fix cascades through hundreds of call sites. Closes `color-contrast` (token swap), `button-name` (aria-label injection), `label` (for/id wiring), `v-html`-without-sanitize, raw library components, hardcoded translations, `target=_blank` without `rel=noopener`, empty silent catches. Skips human-judgment bugs. Re-runs `/ui-crawl` in verify mode for gap-count parity (diffing a pre-fix snapshot, and a before/after visual diff for pixel-changing classes — `VERIFIED`/`SKIPPED`, never asserted). Clean-tree precondition; i18n-key-resolves-in-every-locale gate. Inherits closure-verb discipline from `align-discipline.md` (align pack). Flags: `<class>`, `--dry-run`, `--plan`, `--safe-only`, `--verify`, `--no-commit`, `--module=`. Frontend stacks only. |

**UI-UX skills:**
- `design-canvas` (v1.27+) — draft a surface / flow / feature as **artboards on one pan/zoom canvas**, **before any product code exists** — a link you send to a client, designer or founder and get "yes, but move that" back, instead of prose they have to picture. The visual form of `/redesign`'s and `/art-direct`'s approval gate, and the only entry point that works on a surface with **no code yet**. Writes solely under `ai/design/canvas/` — never a component, token file or route. Asks one question and refuses to infer it: **`system`** (lift your resolved token + component literals and match exactly) vs **`independent`** (free of the repo — landing pages, greenfield). Draws the **states** (empty / loading / error / overflow), not the happy path alone. Ships a self-contained `canvas.html` that opens in any browser; in Claude Code it also upgrades to the bundled WYSIWYG canvas when present.
...
| `/migration-sync [<scope>]` | **Incremental V1→V2 sync — the live-V1 companion to `/migrate`.** Pulls `v1_root` from its remote (halts on a dirty V1, detached HEAD, failed fetch, or a local ahead of the remote), lists the commits past the watermark in `ai/migration/sync-state.md`, classifies each (`behaviour` / `refactor-only` / `dead-v1` / `mechanical` / `needs: full-port`), writes an analysis of what each change *means* (rules, validations, DTOs, endpoints, schema, API-vs-Web), extends `ai/migration/mapping/<feature>.md` BEFORE editing, then dispatches `/find-and-fix` / `/port-feature`. `changes_in == changes_closed` over three buckets only; the final audit re-reads the raw V1 diff; the watermark advances by contiguous prefix and never past an unresolved commit. |
...
| Command                | Purpose                                                                          |
|------------------------|----------------------------------------------------------------------------------|
| `/align-scan`          | Deep scan. Runs 11 universal detectors (6 structural + 5 functional), plus stack-conditional detectors for the `stack-specific` class. Builds `ai/align/ledger.md` (every row `detected`), `scan-report.md`, `findings.md`. Frontend stacks auto-dispatch UI/UX detectors. Security findings always ≥ standard tier; critical security always heavy. |
...
| `/align-park <id> [reason]` | Defer a hairy finding. Sets `status: parked` with rationale plus `prior_status` / `prior_phase` / `parked_sla_from`; excludes from phase gate. Reversible via `/align-unpark`. **Refuses a `class: security` row at critical severity** unless `--override-critical` is passed and recorded — parking one otherwise silences the 24h escalation clock, which keys on `halted`. |
