...
- `setup-project --refresh` (M35) — `run-preflight.sh` takes the Phase 0 backup deterministically; `audit-setup.sh` C2a refuses success without it, and C2k regenerates the study report post-apply and refuses success while any actionable row is neither APPLIED nor RECORDED in `.claude/_refresh-decisions.md` (`--reject` / `--keep-ours` / `--resolve` / `--keep` with rationale). Kept/resolved rows re-open automatically when the pack source changes (`pack@sha8` mismatch). **C2l** warns when a pack **command** was rejected on capability-overlap ("repo has" / "covered by" / "handled by" / "v1 has curated") yet leaves no native same-named command and no `→ use /<equivalent>` breadcrumb — so a user typing `/<cmd>` gets nothing (M35 rule 4; observed 2026-06-20 when `/enhance-ui` silently vanished from a consumer project). Out-of-scope rejections ("not applicable", "out of scope") are exempt. **C2s** (warn-only) flags a curated command **kept** over a richer pack counterpart (`KEEP-OURS`) that is shallower than the standard on either axis: **(1) missing safety gates** the pack carries (sampled set: prior-art / new-dependency / intent gate / `## What to do next` / sibling-shape / coverage-gap / secret-scan / change-brief / missing-agent fallback), **and/or (2) thin section structure** — missing ≥3 standard sections the pack has (Phase 2 Organize / Phase 5 Update / Phase 6 Validate / Phase 7 Improve / Failure modes / Output / Invariants) **while also < 55% of the pack's length** (the length gate spares a complete-but-flat command — e.g. a dense audit — from a false flag). The structure axis closes the hole gates alone missed: a kept command can carry every gate yet still be a 90-line stub lacking the decompose/validate/improve/failure-mode/output scaffold (observed 2026-06-21 — a consumer project's `add-feature` had all gates but was 89 lines vs the standard's 402). Preserving the bespoke command is correct; C2s names the missing gates **and** missing sections and recommends deepening to the standard's structure (keep its agents + anchor), so a shallow kept command is never discovered by accident. Bespoke commands with no pack counterpart are never flagged.
...
| `/upgrade-dep` | **One dependency, framework or runtime, from the version you have to the version you named** — any ecosystem (Node, Composer, pip / Poetry, pub, Bundler, Go modules, Cargo, Gradle / Maven, NuGet, SwiftPM). Five classes decide the ceremony (patch · minor · major-lib · major-framework · runtime); **majors are never batched**. Refuses to start without the two things that make a behaviour-preservation claim checkable: the vendor's breaking-change source for the exact span (cite-or-halt — never recollection) and a **parity oracle whose baseline is captured on the pre-upgrade tree**, chosen by `PROJECT_KIND` (suite + boot + route inventory + contract diff for `backend-*`; typecheck + build + route-mount sweep + bundle delta for `frontend-*` / `mobile-web`; **both** platform targets building plus a launch smoke for `mobile-rn` / `mobile-native` (Flutter and the native toolchains alike); exported-symbol diff for a library; golden output for a CLI). No oracle → the cheapest one is proposed, installed and committed first. Commits are staged and never mixed (manifest+lock · codemod verbatim and alone · one per breaking-change item · config re-shape), so `git revert <codemod-sha>` is a real rollback. Runtime class moves every in-repo pin together — version file, manifest constraint, Dockerfile `FROM`, CI matrix, deploy runtime — or halts; a pin outside the repo is named under `Risks:`. | any |
...
`/roadmap` tracks via `ai/roadmap/plan.md` (the phased plan IS the ledger — no separate projection) and shares `--status` / `--refresh` / `--exclude` / `--max-parallel` / `--allow-dirty` / `--dry-run`; it adds `--goal "<intent>"` (state intended-state inline — maps requirements with no code/doc trace yet, even ones that re-define a working feature), `--build [<N>]` (one phase per run, halt at the gate) and `--no-table-stakes`.
...
| `/tdd [<feature>]` | testing | Red→green→refactor TDD loop via the `tdd-orchestrator` agent — failing test first, minimal pass, refactor under green. |
...
**Design modes set the ceiling — this is the "my new theme barely changed" gotcha (v1.23+).** Default and `--skin` are **RE-SKINS**: they mirror the base theme's layout + component set and retune only the tokens (colors / type / radii / elevation), so the result reads modern but stays the *same composition* — same sections, same order, same layout. They will NOT produce a new look-direction. **`--reimagine` is the only mode that produces a genuinely new direction:** it routes the direction through `/art-direct` (invent a language) and re-composes each surface via `/redesign` (new layout, reordered / added section types), inheriting their **hard gates** — the new theme must *beat the base* (rendered side-by-side, win on hierarchy · modern · distinctiveness · craft · appeal), *re-compose* (IA/order/archetype must differ — a mirror-with-new-tokens fails), and carry one *named loud move*, or it HALTs. Because a new theme is additive, `--reimagine` may **add** new section types the base never had; **parity is FEATURE parity (no capability dropped), NOT layout parity** — so re-composing and adding never fail parity, only *dropping* a base capability does. If the ask is a new/bold look but `--reimagine` wasn't passed, the Frame step recommends it rather than silently shipping a recolor.
...
| `validate-pack-consistency.sh` | **Pack-manifest gate** — per pack: the three sync files exist, `_version.json` parses, every `_topics` `fallback:` resolves *and names a strategy that can deliver something* (**check 3b**, below), every `_essentials` entry resolves, artifacts have topic entries, the current version is in the changelog, and **check 8b** holds every `_examples/` fallback to the source it abridges (below). |
...
### Fallback strategy — `validate-pack-consistency.sh` check 3b

Check 3 asks whether a topic's `fallback:` **value resolves**. Check 3b asks whether the **strategy it names can deliver anything**, which is a different question and the one that kept going wrong.

Two values are legal: a path inside the pack, or the literal `stub-from-sections`, which [`templates/phases/phase-4.2-apply.md`](../templates/phases/phase-4.2-apply.md) defines — in the only sentence in the tree that defines it — as emitting "a sectioned stub from that topic's `sections:` list". Declare that sentinel with **no `sections:` key** and the list is empty, so the emitter emits an **empty file**, usually with a finished artifact of the same name sitting beside it on disk. Nothing downstream notices: the phase-4.0 depth floor measures the *pack source*, the phase-4.0 minimums count *files* (a 0-byte file counts as present), and phase-5's populate gate covers only the foundational `ai/` set. The size of an emitted pack artifact is measured nowhere else.

That defect recurred **five times** — `security`, `infrastructure`, `distributed-systems`, eight skill topics across `data-engineering` + `finops`, and finally `observability`'s four (855 finished lines the topics were refusing to deliver) — hand-repaired every time and gated none of them, which is why it kept coming back.

| Rule | Severity | Meaning |
|---|---|---|
| `STUB-NO-SECTIONS` | **FAIL, not ledgerable** | The sentinel with no **non-empty** `sections:` list — a missing key, a `sections: []` and a bare `sections:` with nothing under it are the same thing. It cannot emit anything, so there is no legitimate instance and nothing for a reader to decide. A ledger line claiming to suppress one WARNs and suppresses nothing. |
| `STUB-OVER-SOURCE` | **FAIL if new**, ledgered | The sentinel where a finished artifact of the same `kind:` and name already ships in the pack. This *is* a judgement call — a pack may prefer a neutral skeleton to a generic file — so it is ratcheted through [`templates/packs/_topics-strategy-baseline.md`](../templates/packs/_topics-strategy-baseline.md) with a mandatory `# reason`. |
| (check 3, sentinel arm) | **FAIL** | A `fallback:` that is neither a resolving path nor the exact literal `stub-from-sections`. A one-character typo used to read as a sentinel and pass silently, which is an undetectable *no* fallback. |

`--coverage-report` prints the ledger plus a per-pack greenfield coverage table. **Those percentages are reported and never gated.** Measured across the 23 packs the line ratio runs 48–100% with no gap in it, and it inverts against harm: `ai-engineering` is second-worst at 50% with *zero* zero-delivery topics (its gap is deliberate abridgement), while `observability` sat mid-table at 72% while four of its topics delivered an empty file. What *is* gated is the **count** of zero-delivery topics, per pack, in [`templates/packs/_greenfield-budget.md`](../templates/packs/_greenfield-budget.md) — a number that may not grow, regenerated with `--record-budget`, the same mechanism `check-rule-budget.sh` runs against `_rule-budget-baseline.txt`. That budget is what closes the third shape neither rule above catches: a sentinel that declares `sections:` and has **no** artifact of that name fires neither `STUB-NO-SECTIONS` nor `STUB-OVER-SOURCE`, so before the budget existed a pack could add unlimited zero-delivery topics and stay green. The count is padding-proof where the percentage is not — nothing you *add* lowers it — and its ceiling is per-pack granularity, so a repair plus a regression inside one pack nets to zero. `--record-strategy` regenerates the ledger, carrying existing reasons over verbatim and writing new entries *without* one, so a recorded entry suppresses nothing until a human writes its reason.
...
Check 8b compares every fallback against its source on the axes a text comparison can decide — 282 pairs today. It flags three shapes:
...
2. something the source carries that the corpus keeps **≥85% of the time**, or a **safety signal** at any retention (a load-bearing section, the section order, a `> **Hard rule:` line, a halt block, a `## Premise`, an agent `TRIGGER` clause, an agent's **sibling / cross-pack boundary block**, or the **closing gate / verdict block** that decides whether the run may call itself done);
3. a fallback whose body **is** its source with nothing declaring it (`UNDECLARED-COPY`).

Rule names you will see in the failure line: `COPY-DRIFT` · `UNDECLARED-COPY` · `UNSOURCED-MAGNITUDE` · `DANGLING-DISPATCH` · `FRONTMATTER-LOSS` · `NOT-AN-ARTIFACT` · `SECTION-LOSS` · `SECTION-ORDER` · `SIGNAL-LOSS` · `BOUNDARY-LOSS` · `CLOSING-SIGNAL-LOSS` · `VERDICT-DEGRADED`.

The last three were armed together at **zero baseline lines**, then widened and re-measured. Read them with their coverage attached rather than as blanket protection: `CLOSING-SIGNAL-LOSS` and `VERDICT-DEGRADED` evaluate the **80 of 282 pairs (28%)** whose *source* carries a done-condition — the other 202 sources do not close on one, so there is nothing for a fallback to drop — and the hard-FAIL half of `BOUNDARY-LOSS` is scoped to the `agents` class, where 77 of 77 sources carry the block. Outside that class the same loss is real but the convention is not universal, so it is **counted and budgeted per pack** (51 findings across 12 packs today) in [`templates/packs/_greenfield-budget.md`](../templates/packs/_greenfield-budget.md) rather than baselined with 51 reasons nobody would read; `--fallback-report` lists every one. `CLOSING-SIGNAL-LOSS` earned its place from a measured incident: `ai-engineering/_examples/add-ai-feature.md` never gained the `## Ship gate` its source added, kept a `Status: COMPLETE` the source forbids in two places, and survived 13 commits / 44 days / a full minor release — *including the commit that armed check 8b*, which exited 0 over it and never named the file. It sat on that pack's only `_essentials.md` command, so it was what every greenfield AI project received.

**It is a ratchet, not a bare FAIL.** Fallbacks are deliberately abridged, so a "must match source" rule would flag hundreds, get muted, and be worse than no gate. Known-and-accepted violations are enumerated in [`templates/packs/_fallback-baseline.md`](../templates/packs/_fallback-baseline.md) and suppressed into one counted WARN; **anything not listed is a hard FAIL**. The backlog is **zero**, and that file is the authority on its own size — a claim the gate now checks rather than trusts: 8b parses the file's "The backlog is N lines" sentence and FAILs when it disagrees with the entries that parse, because that sentence and the check's own header comment both went stale the day the backlog emptied.
...
git clone <remote> ~/Workspace/Projects/refract
