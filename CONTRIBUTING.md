...
`.github/workflows/quality-gates.yml` runs **62 blocking steps** on every push to `main` and every
pull request. Every one of them is blocking: a red gate is a merge blocker, not a note for later.

> **All 62 gates are green on `main`.** There is no known-red allowance: if a gate fails locally,
...
| `tests/hooks/run.sh` | A security hook regressed to always-allow. 76 fixtures across `guard-destructive` (23), `module-boundaries` (24), `pre-edit-guard` (11), `secret-scan` (8), `inject-path-rules` (5), `recall-inject` (5), plus 3 programmatic assertions that `module-boundaries` stays inert with no `ai/modules.md`, an empty boundaries section, or the opt-out flag, plus 10 payload assertions for the context-only `inject-blast-radius` (exit code proves nothing there — a hook that exits 0 saying nothing looks identical to one that works) — 89 checks total. Filename encodes the expected exit: `*block*` → 2, `*allow*` → 0. | Fix the hook. If the new behaviour is correct, add a fixture proving it. |
...
escapes the temp root. Extend it rather than testing by hand: it is one of §2's 62 blocking gates,
