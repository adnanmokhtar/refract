...
`.github/workflows/quality-gates.yml` runs **59 blocking steps** on every push to `main` and every
pull request. Every one of them is blocking: a red gate is a merge blocker, not a note for later.

> **All 59 gates are green on `main`.** There is no known-red allowance: if a gate fails locally,
...
escapes the temp root. Extend it rather than testing by hand: it is one of §2's 59 blocking gates,
