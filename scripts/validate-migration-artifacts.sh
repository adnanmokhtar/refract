...
check_section_0_evidence() {
  ...
  local diff_rows
  diff_rows=$(awk '/^#{2,4}[[:space:]]+Section 0[[:space:]]*[-—][[:space:]]*Leaf[- ]?set diff/,/^#{2,4}/' "$file" 2>/dev/null | grep -cE '^\|.+\|.+\|.+\|.+\|' | tail -1 || echo 0)
...
check_per_axis_enumeration() {
  ...
  while IFS= read -r cand_v1; do
    ...
    local form_hits
    form_hits=$( { grep -cE '<input\b|<v-text-field|v-model=|<InputText\b|<InputSwitch\b|<InputNumber\b|<Dropdown\b|<TranslatedInput\b|<FormField\b|<Textarea\b|<Select\b|<Checkbox\b' "$vf_res" 2>/dev/null || echo 0; } | tail -1 )
...
check_adr_signoff_completed() {
  ...
  for adr in $adr_refs; do
    ...
    while IFS= read -r adr_file; do
      ...
      unchecked_count=$(echo "$signoff_block" | grep -cE '^[[:space:]]*-[[:space:]]+\[[[:space:]]\]' | tail -1 || echo 0)
      checked_count=$(echo "$signoff_block" | grep -cE '^[[:space:]]*-[[:space:]]+\[[xX]\]' | tail -1 || echo 0)
