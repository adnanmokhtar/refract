...
if [[ "$MODE" == "refresh" || "$MODE" == "refine" || "$MODE" == "enhance" ]]; then
  ...
  if [[ "$C2N_LINE_MODE" -eq 1 ]]; then
    ...
    ok "loss test: token fallback — $( { grep -c . "${C2N_PACK_IDX:-/dev/null}" 2>/dev/null || echo 0; } | tail -1 ) pack token(s), $( { grep -c . "${C2N_REPO_IDX:-/dev/null}" 2>/dev/null || echo 0; } | tail -1 ) project-source identifier(s)"
  fi
  ...
  while IFS= read -r bkdir; do
    ...
    while IFS= read -r bf; do
      ...
      case "$rel" in
        # 🔴 THE THREE COMPACT PROJECTIONS ARE REGENERATED BY CONTRACT, SO A REPLACED LINE IN ONE
        # IS NOT A LOSS — AND CHARGING FOR IT MAKES TWO CHECKS CONTRADICT EACH OTHER.
        #
        # commands/setup-project.md § "Token efficiency" declares these three "REGENERATED, not
        # hand-edited. Source of truth is the full files; compact files are derived projections."
        ...
        # FULL sources the projections derive FROM and stay protected: a fact dropped there is
        # gone, while a fact dropped from a projection still lives upstream and comes back on the
        # next regeneration. Gate: test-audit-setup-c2n.sh.
        ai/_session-digest.md|ai/_convention-cheatsheet.md|ai/_decision-index.md) continue ;;
...
c2y_code_probes() {
  ...
  # 🔴 A PROBE INSIDE A DATED RECORD IS NOT THERE TO BE RUN TODAY.
  #
  # `ai/audits/`, `ai/decisions/` and `ai/failures/` record what was true at a moment — two of
  # them append-only by contract. The command lines quoted in them are EVIDENCE of a past run,
  # not instructions for the next one, and the directories they name may belong to a different
  # system entirely.
  #
  # 📏 On the migration repo in the corpus, `ai/audits/parity/*` compares V2 against V1, so its
  # probes name V1's `supabase/`, `features/`, `orders/` — 52 hits, every one correct as a
  # record and none of them a defect here. Worse, treating them as defects invites the repair:
  # a retarget pass rewrote 20 of those files before this exclusion existed, turning "we checked
  # V1's src/" into "we checked V2's apps/" — falsifying findings to satisfy a check. The
  # exclusion is the same one scripts/retarget-probes.sh applies, for the same reason.
  { grep -rIl '' "$CL/commands" "$CL/agents" "$CL/skills" "$CL/rules" "$TARGET/ai" 2>/dev/null \
      | grep -vE '/backups/|/ai/audits/|/ai/decisions/|/ai/failures/' || true; } | sort | while IFS= read -r _file; do
    [[ -f "$_file" ]] || continue
    awk -v F="$_file" '
      /^[[:space:]]*(```|~~~)/ { fence = !fence; next }
      {
...
while IFS=$'\t' read -r loc code; do
  ...
  while IFS= read -r dir; do
    ...
    # 🔴 A SLASH IS NOT A PATH. `tag/component` IS ENGLISH.
    #
    # The extractor concatenates every inline `…` span on a line so a command split across two
    # spans still reads as one command — deliberate, and it is why prose written inside code
    # spans can reach this loop. 📏 Four survivors across seven repos were compounds, not paths:
    # `tag/component`, `derived/computed`, `tenant/ACL`, `try/catch/finally` (the last one 72
    # times in one repo, because an anchor block copied it into every artifact).
    #
    # A path an operator actually typed carries one of three marks: a trailing `/` (a directory),
    # a file extension, or a glob. None of the four has any. The recall this gives up is a probe
    # naming a directory with no trailing slash — a shape the packs do not use, and the cheapest
    # thing on the table next to a check that cries wolf on English.
    case "$dir" in
      */) ;;                              # directory
      *\**) ;;                            # glob
    ...
    # …and the same sentence applies to a path naming a DIFFERENT sibling, which the self-name
    # test above cannot see. 📏 On one repo of a seven-repo workspace, 5 of the 8 surviving hits
    # were `product-theme-pages/…`, `store/…` and `<sibling>/src/modules/…` — every one of them
    # resolving one directory up, every one correct for a reader working from the shared parent.
    # Rewriting them repo-relative would make them WRONG, so the check must not demand it. Only a
    # directory that is a real sibling counts; an invented name still fails.
    [[ -d "$TARGET/../$root" ]] && continue
    [[ -e "$TARGET/$root" ]] && continue
    c2w_n=$((c2w_n + 1))
    c2w_hits="$c2w_hits"$'\n'"      ${f#$TARGET/}:$lno probes \`$dir\` — no \`$root\` in this repo"
    break
    # A URL is not a probe target. 📏 `https://www.w3.org/WAI/WCAG22/Understanding/…` in a
    # pattern file was harvested as a path rooted at `org/`, and the repo was told it lacked a
    # directory nobody ever meant. Strip URLs before harvesting rather than trying to recognise
    # them afterwards — by then the scheme is gone and `org/WAI/...` looks like any other path.
  done < <(printf '%s\n' "$code" | sed -E 's#[a-z][a-z0-9+.-]*://[^[:space:]"'"'"'`)]*##g' \
            | grep -oE '(^|[[:space:]])[A-Za-z0-9_][A-Za-z0-9_.*-]*/[A-Za-z0-9_./*-]*' 2>/dev/null | sed 's/^[[:space:]]*//')
