...
- [ ] New sensitive field (credential / payment instrument / government id / health record / third-party token) is captured through the provider's primitive and stored as a token — not an input bound to the repo's schema, and not a column, filter, fixture, log or analytics property.
