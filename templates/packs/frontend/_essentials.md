...
- skills: visual-check is the only way to actually verify rendered UI — indispensable. The performance-specialist scanners (ssr-audit, lighthouse-ci, bundle-analyze, streaming-ssr, navigation-speed, lcp-audit) plus seo-audit (technical-SEO scanner, paired with the technical-seo agent) ship in standard mode — invoked when a perf / SSR / navigation / SEO concern surfaces, not on every minimal scaffold.
