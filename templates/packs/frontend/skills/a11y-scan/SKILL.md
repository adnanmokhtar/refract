---
name: a11y-scan
description: Automated accessibility scan using axe-core against a running app; catches roughly 30% of real issues and blocks merge on critical pages. Needs a running server — start one with `dev-server-start` first. Automated coverage only — the remaining ~70% needs the manual pass in `a11y-quick-check` or the frontend pack's @accessibility-auditor.
---
