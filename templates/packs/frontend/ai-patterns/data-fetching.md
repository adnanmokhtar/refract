...
- `pagination.md` · `api-contract.md` *(backend pack, when co-installed)* — the server side of paginated reads and field projection (Detector 7's hand-off target), and the source of the two values this pattern's paginated cache keys are built from: the exact `meta` keys (`nextCursor` / `hasMore` for cursor, `page` / `pageSize` / `total` for offset) and the exact query-param spelling. Read them off `api-snapshots/README.md` when the API publishes one. **Absent that pack and that file** → read the spelling off the endpoint or one real list response and record `pagination shape: derived from response`; never key a cache on a `meta` member you assumed, because the miss is `undefined` rather than an error and shows up as a list that renders once and never pages.
- `@api-contract-sentry` (in-pack agent) — reads the contract this pattern's queries are written against. First delivery: it reports the shape before the query exists. Change: it reports which queries break. Its read of the published baseline outranks any shape restated here.
