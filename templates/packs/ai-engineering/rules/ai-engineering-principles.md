...
- **Standing whole-surface sweep:** `/ai-audit` audits AI-1…AI-9 across six axes (eval coverage, prompt, retrieval, ANN index, agent budgets + tool gates, gateway/cost/observability) on an existing surface, dispatching the pack's detector skills and routing the trust boundary to `@llm-security-reviewer`. It may never print a green verdict while the eval axis is UNVERIFIED. (all)
- **Mechanical checks behind the principles:** `prompt-audit` for AI-4 / AI-5 / AI-7 (structured output, determinism, prompt versioning); `llm-gateway-audit` for AI-3 / AI-8 / AI-9 (token cap + timeout + traced cost, one seam, redaction at the seam); `vector-index-audit` + `retrieval-eval` for AI-6 (tenant/permission filter at the store, retrieval quality measured, the index tuned to a *stated* recall target — the audit reports whether the target was declared, the eval produces the number, and neither invents one).
- **No harness ⇒ retrofit, not a waiver:** when `eval-run` HALTs with "no eval harness", `/add-eval-set <feature>` builds the versioned dataset + scorers + declared threshold + committed baseline + CI gate. Until that first measured run exists the feature's eval axis is UNVERIFIED, never a faked pass. (AI-2)
...
- **Agents (in-pack):** `@ai-feature-reviewer` (grades a diff across eval / prompt / RAG + index / agent safety / cost-latency / guardrails / fine-tune justification), `@rag-architect` (designs the retrieval pipeline and the ANN index against a stated target before code exists — AI-6), `@agent-loop-architect` (picks the autonomy rung, the tool gates, and the four loop budgets, and argues a loop down into a workflow where one suffices — AI-1, AI-3).
- **Skills (in-pack):** `eval-run` (runs the harness and gates on regression — AI-2), `prompt-audit` (AI-4/5/7), `llm-gateway-audit` (AI-3/8/9), `retrieval-eval` (AI-6, the measurement), `vector-index-audit` (AI-6, the index configuration).
- **Commands (in-pack):** `/add-ai-feature` (builds), `/ai-audit` (audits an existing surface), `/add-eval-set` (retrofits the regression gate).
