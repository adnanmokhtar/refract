...
- **`temperature > 0` on extraction/classification** MUST be cited.
...
**Temperature 0** (or lowest) for extraction, classification, routing, structured output, tool-argument generation — single-answer tasks. Non-determinism there is a correctness/reproducibility bug and makes evals noisy. **Fix the seed** where supported (best-effort). Higher temperature only where variation is the point. Determinism makes the eval gate meaningful — a scorer over a flapping output can't tell a regression from noise (see `evals`).

## 6. Versioning — prompts are code

**Version every production prompt** (id + version) so an output is attributable to prompt-version × model × params. **Never edit in place with no trail** — bump the version, run the eval set, review the eval-diff on the PR (see `evals`). **Pin the model + params with the prompt** (only reproducible together); a model upgrade re-runs the eval set first. Centralize prompts (not scattered inline literals) so they're reviewable/diffable/testable.
...
- **`temperature > 0` on extraction/classification** → `set-deterministic-params`
