...
2. **Prompt quality** — structured output (tool/JSON-schema, `strict` variant where offered), not regex-parse; instruction/data separated; the single-answer constraint uses whatever the provider still exposes (`temperature: 0` where sampling params exist — where the provider withdrew them a temperature set at all is a **BLOCKER**, since the call 400s rather than degrades; read the model id first). BLOCKER: regex-parsing structured data.
