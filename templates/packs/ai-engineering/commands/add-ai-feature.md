...
5. **Security handoff — dispatch `@llm-security-reviewer`** for the trust boundary: prompt injection (direct + indirect), improper output handling (any sink the output reaches), and excessive agency (destructive tools). This is a required handoff, not optional — `@ai-feature-reviewer` reviews engineering quality but does NOT clear security. `@llm-security-reviewer` ships in the **security** pack. Absent that pack there is no addressee: the row is emitted as `HANDOFF UNROUTED (security pack not installed)` carrying its `<path:line>` and OWASP-LLM id, and `/setup-project --refresh --include=security` is named as the way to route it. An owner that is not installed never turns an unowned finding into a cleared one.
...
- **`@llm-security-reviewer`** *(security pack, when co-installed)* — the trust boundary (prompt injection / output handling / excessive agency). REQUIRED whenever user content enters the prompt, output reaches a sink, or the model can call a destructive tool. Absent the pack the requirement does not lapse — it becomes an `UNROUTED` row on the Phase 5 line below.
...
Phase 5 (Evaluated): eval set v1 (<N> cases) built + gated in CI; eval-run measured <metric>=<score> vs ≥<threshold> (new: absolute bar / change: baseline−ε); @llm-security-reviewer handoff cleared *(or, on an install without the security pack, `handoff UNROUTED — security pack not installed`; the phase closes as INCOMPLETE on that axis rather than claiming a clearance nobody gave)*.
