...
> **Validator parity — closed.** `scripts/validate-polish-artifacts.sh` routes `mobile-*` through `check_frontend_verb_vocabulary`, and that check now accepts the five mobile verbs on top of the 19 `ui-design-sweep` ones (`MOBILE_EXTRA_VERBS`, gated on `PROJECT_KIND=mobile-*` so a frontend project still cannot use them). The registration in `commands/polish.md` § Mobile is the contract and the validator agrees with it; a vocabulary failure on mobile is now a real finding defect, not a known gap.
