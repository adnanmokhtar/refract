...
- skills: secret-scan catches the most common, highest-impact accident (committed credentials). deps-audit (CVEs, EPSS/KEV-triaged), threat-model, and ssrf-scan (SSRF-egress detector: user-URL → outbound fetch, metadata-IP/DNS-rebinding, + security-specific upload risks) ship in standard mode.
