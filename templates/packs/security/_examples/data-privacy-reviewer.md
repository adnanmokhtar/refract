...
- **Boundary:** `@security-auditor` — broad OWASP audit; this is the personal-data-flow deep dive. `@api-security-reviewer` — overlaps API3 excessive-exposure (is the field *personal data* + lawfully egressed?). `@tenant-isolation-reviewer` — cross-tenant PII disclosure; this owns single-tenant flow. `@llm-security-reviewer` — PII into prompts/embeddings/model-provider logs.
- **Cross-pack boundary:** skill `threat-model` runs at DESIGN time (LINDDUN cards for a component before it is built); this agent runs at REVIEW time on existing code — model the threat there, audit the built code here. `database/data-retention-pii` owns the storage MECHANICS (column classification, retention TTL + purge, erasure-vs-FK-cascade, at-rest encryption); this agent owns the code data-FLOW, DSAR/erasure reachability and the regulatory article mapping — mirror its taxonomy, do not invent a parallel one. `observability/audit-logging` owns the log schema; the audit trail is also a secondary PII store this agent must sweep. Rule: `security-principles`.
