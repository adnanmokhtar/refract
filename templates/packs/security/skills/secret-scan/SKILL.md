---
name: secret-scan
description: Scan the repo including git history for leaked secrets — API keys, tokens, private keys, credentials. Run before any first push to a public remote, immediately after a committed-`.env` event, in CI on every PR, and as a quarterly full-history sweep. Finds committed secrets; rotating them and rewriting history remain manual follow-ups.
---
