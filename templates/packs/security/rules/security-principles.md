...
- **The carve-out that breaks that rule.** Bind parameters cannot carry a table/column identifier, a sort column, `ASC`/`DESC`, or (in most drivers) `LIMIT`/`OFFSET`. Map those through a server-side allow-list — `{"newest": "created_at"}` → the mapped value, else reject — never through the request string, and decide it once in the query helper. Every `?sort=` endpoint lands here. (OWASP *SQL Injection Prevention* § "Defense Option 3: Allow-list Input Validation" — https://cheatsheetseries.owasp.org/cheatsheets/SQL_Injection_Prevention_Cheat_Sheet.html)
...
- File uploads: validate type by magic bytes (not the client-sent extension / MIME), cap size, store outside the webroot (or object storage) with a generated name — never serve or execute an upload from a path the server interprets. See the backend `file-upload` pattern.
...
- Where the app collects PII/PHI: every field's sinks (store → log → analytics → third-party egress) are known and consented, erasure/DSAR reaches **every** sink not just the primary row, and cross-border transfer is checked against the *configured* regime, never a defaulted one. Flow + law: `@data-privacy-reviewer`. Tagging/TTL/cascade/at-rest: database `data-retention-pii`.
...
- Guard the *flow*, not only the route: a multi-step flow enforces its own ordering server-side (no skipping payment, no replaying a completed step), and a step that is harmful when scripted (bulk purchase, coupon redemption) needs anti-automation beyond a generic limiter.
