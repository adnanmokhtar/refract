...
- **Rate limiting** — `golang.org/x/time/rate` (token-bucket `Limiter`, keyed per client in a map+mutex or an LRU), or a router middleware (`go-chi/httprate`, `ulule/limiter`). `x/time/rate` is **in-memory per process** — behind >1 instance each replica has its own bucket, so use a Redis-backed limiter (`ulule/limiter` store) for a real global limit. On reject, set `Retry-After` + the current draft's two quota fields — `RateLimit-Policy: "default";q=100;w=60` and `RateLimit: "default";r=0;t=30` (`draft-ietf-httpapi-ratelimit-headers`, a DRAFT) — then `http.Error(w, ..., http.StatusTooManyRequests)` (429). All headers are manual here, so there is nothing to migrate off: write the two-field form, and add the legacy `RateLimit-Limit/Remaining/Reset` triple only if a client still reads it. → `ai-patterns/rate-limiting.md`.
