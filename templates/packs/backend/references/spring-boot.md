...
- **Rate limiting** (inbound, per-tenant): Bucket4j with per-key buckets (`ProxyManager` over a shared store — Redis/Hazelcast — for multi-instance), or Resilience4j `RateLimiter`. Wrap in an `OncePerRequestFilter`/`HandlerInterceptor` keyed on tenant/API-key; on miss return `429 Too Many Requests` (RFC 6585) + `Retry-After` (RFC 9110 §10.2.3) + the two quota fields the current draft defines — `RateLimit-Policy: "default";q=100;w=60` and `RateLimit: "default";r=0;t=30` (IETF draft-ietf-httpapi-ratelimit-headers — a DRAFT, not an RFC). The `RateLimit-Limit`/`-Remaining`/`-Reset` triple is draft-05 legacy; emit it alongside only while clients still read it. Decide FAIL-OPEN vs FAIL-CLOSED on store outage; emit `503` for admission control. → see `../ai-patterns/rate-limiting.md`.
