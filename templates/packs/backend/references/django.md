...
- **Rate limiting** → [rate-limiting.md](../ai-patterns/rate-limiting.md). Use DRF `ScopedRateThrottle` (per-`throttle_scope` buckets) or `django-ratelimit`; emit `429 Too Many Requests` (RFC 6585) + `Retry-After` (RFC 9110 §10.2.3) + the two quota fields the current IETF draft defines — `RateLimit-Policy: "default";q=100;w=60` and `RateLimit: "default";r=0;t=30` (`draft-ietf-httpapi-ratelimit-headers` — a DRAFT, not an RFC). Neither throttle class emits them; set them in the exception handler. The `RateLimit-Limit/Remaining/Reset` triple is draft-05 legacy — keep emitting it alongside only while clients still read it.
