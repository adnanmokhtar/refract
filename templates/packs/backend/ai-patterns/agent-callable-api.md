...
## Descriptions are the interface — and this repo already knows it

**Draw the parallel explicitly, because it is exact.** This framework's own commands are selected by an agent reading one description line. `templates/canonical-command-template.md:35` demands a description whose first ≤90 characters front-load the matching keywords, and lines 42–44 require an explicit `USE:` / `NOT:` pair — trigger scenarios and anti-scenarios naming the command to use instead. Every skill and command in this repo is written that way *because routing accuracy collapses without it.* A tool description is the same artifact under a different name, routed by the same mechanism, and it fails the same way.
