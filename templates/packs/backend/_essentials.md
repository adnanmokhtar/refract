...
- ai-patterns: api-contract (request/response shape), error-handling (must-have safety pattern), parallel-io (cites the project's actual concurrency primitive — without it, agents reach for generic `Promise.all` and miss bounding / cancellation / batch APIs), rate-limiting (load-bearing inbound safety — every public/expensive endpoint needs an enforced 429 limit; minimal mode ships it). conditional-requests (ETag/optimistic-concurrency) ships in standard mode; response-streaming and async-job-offload are signal-gated (large-result / background-job endpoints) and ship in standard mode. Multi-tenancy lives under `templates/domains/multi-tenant/` (signal-gated), not in the backend essentials.
