...
| Endpoint fetches a user-supplied URL | SSRF egress allowlist: block private / link-local / loopback / cloud-metadata ranges, https-only, validate the *resolved* IP (re-resolve, defeat DNS-rebind), no redirects to denied hosts. e2e rejects a metadata-IP (`169.254.169.254`) and a loopback target. Owned by **security-auditor (OWASP A01:2025 Broken Access Control, which absorbed SSRF / CWE-918 — it was A10:2021)** — pointer, not duplicated policy. (SEC-02) |
