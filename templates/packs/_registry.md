...
Not every `pack:` frontmatter value names a track. The top-level core commands in `commands/` (`/optimize`, `/align`, `/polish`, `/audit`, `/refactor`, `/roadmap`, `/do`, `/scaffold-project`, `/refine-prompt`, `/task`, `/setup-project-adapters`, `/setup-project-health`, `/unify-surfaces`, `/upgrade-dep`) carry `pack: orchestration`. This is a **logical grouping label**, not a track:
