...
#### CHANGE-anchor / CHANGE-anchor-with-warn / ANCHOR-DEEP — anchor block shapes

The two block shapes, their required HTML markers, the round-one vs round-two structure, and the
identifier-traceability rule Phase 5.3.5 enforces: **`references/anchor-block-shapes.md`**. Read it
when you ACT on one of these three decisions — the templates are long, exact, and needed only then.

Non-negotiable regardless: every Phase 4.6 anchor emits the `<!-- project-specific:start -->` /
`<!-- project-specific:end -->` markers at column 0, and every identifier inside the block must be
findable in the extraction artifacts or Phase 5.3.5 HALTs the run as a cross-project leak.
...
2. **`.claude/_phase-4-6-decisions.md`** — decision log for Phase 5.3 audit. Round-one and REFINE rows go under separate sections to keep history readable:
   ...
   | `.claude/skills/parallelize-independent-ops/SKILL.md` | LEAVE-DEEP-IDEMPOTENT | 84 → 84 | (cached) | (matches prior run) | hashes match prior REFINE; no new signal |
...
- **ADAPTER-SYNC — adapter `_translate.md` missing**: not a halt — falls back to Phase 4.8.0's per-adapter contract table (the canonical translation rules live in `commands/setup-project-adapters.md` § "Per-adapter minimum output" anyway). The skill records `FALLBACK-TO-PHASE-4.8-CONTRACT <adapter>` in the decision row notes column.
