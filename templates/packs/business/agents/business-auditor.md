...
- `@domain-model-auditor` — structure-vs-UX complement. This agent audits the **experience** (does the user find the flow, recover from errors, complete the cycle); domain-model-auditor audits the **aggregate + invariant structure** (is `Order` a real consistency boundary or a data bag, does each invariant name its enforcement layer). business-auditor asks "can the user cancel?"; domain-model-auditor asks "is `Subscription`'s invariant enforced anywhere?" Run both on a rich-domain feature.
- `pricing-tax-audit` — money-MATH complement to this agent's billing-UX checklist. business-auditor confirms the money *features* exist (invoices downloadable, dunning flow, refunds reachable); pricing-tax-audit confirms the *arithmetic* is right (no float, rounding/jurisdiction/currency explicit, idempotent charges). Run both on a billing feature.
