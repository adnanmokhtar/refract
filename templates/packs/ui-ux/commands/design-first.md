...
The failure it prevents is the one that costs the most: the twelve-command map in `ui-sweep.md` is correct and complete, and someone who does not already know the map picks by keyword, lands on `/enhance-ui` when they wanted a redesign, and gets a tidier version of the layout they were trying to throw away.
