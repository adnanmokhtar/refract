...
**Feature-parity gate (no dropped component — especially in `reimagine`).** A new visual direction redraws every component; it does NOT delete any. The build inherits `/redesign`'s Phase-6 parity gate: the Phase-1 component/feature manifest (every KPI card, chart, table, filter, control, action) is diffed against the rebuilt render item-by-item, and a single MISSING component — the classic reimagine failure of dropping the conversion-rate card or a table to "simplify" — is `INCOMPLETE`, never a silent omission. Counts must match (4 cards in → 4 cards out). The agent NEVER drops a component on its own; a drop is legitimate only when the user explicitly approved it (keep/move/drop). reimagine's freedom is over *form* (re-rank, re-group, demote, re-shape) — never over the *feature set*.
