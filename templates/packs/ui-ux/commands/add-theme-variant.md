...
### Pillar 4 — Feature-complete (parity vs the DEFAULT theme) — HARD GATE
The new theme must provide **everything the default theme provides**, measured against the project's model:
- **Model B (per-theme components):** every **component**, every **icon** (card / action / section-header, empty-state illustrations), every **rendered state** (loading / empty / error / success / zero), and every **layout** the default theme ships. A missing component does not error — it **silently falls back to the default theme's component**, which renders off-language and off-brand (an invisible bug, not a missing feature).
...
2. **Inventory (the parity contract)** — read the DEFAULT theme off disk: enumerate its component / icon / state / layout set into the **parity manifest** (counted), and record its slot structure, token source, and the project's theme-resolution mechanism (glob / whitelist / resolver) + SSR rule + RTL setup from `_extracted-idioms.md` + `ai/patterns/theming.md` + the cited architecture rule. This manifest is the non-negotiable reference for the Parity gate.
3. **Design the new theme's tokens** — default: a cohesive modern token system; `--reimagine`: run `/art-direct` scoped to the new slot (concept → direction → tokens); `--skin`: a modern token refresh off the default's structure. Color roles · type scale · spacing rhythm · radii · elevation · reduced-motion-safe motion.
4. **Build the slot (additive only)** — create the new theme's directory parallel to the default: its tokens, its component styles (mirroring the default theme's component set, styled to the new tokens), its layouts / style-entrypoints / config. Apply the project's performance conventions to these components. **Only new paths under the new theme's dir are written.**
5. **Register (append-only)** — wire theme resolution the way the detected mechanism expects: glob auto-discovery → often nothing to edit; whitelist/enum → append the slug; resolver → extend additively. Never a rewrite.
6. **Run the four gates** — Additive, Architecture, Parity, Perf (below). Any HALT stops the run with done-vs-pending.
7. **Verify + report** — render the new theme across the project's key surfaces × every locale (incl. RTL) × mobile + desktop; confirm all four gates green; emit the brief report + honesty footer.
