...
> **Not this command? (ANTI-triggers)** — the project has ONE theme and no slot system → **`/art-direct`** (design an identity) · **`/redesign`** (rebuild a surface); this command HALTs, because there is nothing to parallel. Change how ALL themes look → **`/art-direct`**. An existing theme has drifted from the base and you want the gaps found → the **`theme-specialist`** agent in its parity-audit mode. Rebuild one surface's layout inside the current theme → **`/redesign`**. Add finish inside the current theme → **`/enhance-ui`**. **This command only ever creates the (N+1)th slot; if a request needs an existing file changed, it halts rather than obliging.** Full map: [`ui-sweep.md § The ui-ux command map`](ui-sweep.md).
...
- The project's **performance check** identified — **`/perf-audit`** *(performance pack, when installed)* or the repo's own budget script / CI perf job. Name the one this repo actually has; if it has none, the Perf gate is `SKIPPED (no perf check)`, stated plainly, never faked green. (There is no `/perf-check` command in this repo — an earlier version of this file named one, and a gate cleared by running a command that does not exist is a fabricated pass.)
...
- **Perf gate.** Run the project's perf check — **`/perf-audit`** *(performance pack, when installed)* or the repo's own budget script — on the new theme's key routes, on mobile. A breach of the project's budget → HALT. No perf check in the repo → `SKIPPED (no perf check)`, stated, never faked. Never print a budget figure the run did not measure.
...
Perf:       /perf-audit on 3 key routes @ mobile — within budget (measured: LCP 1.9s ·
            bundle +6KB)   [reads SKIPPED (no perf check) when the repo has none —
            the numbers here are an example of the SHAPE, never a default to print]
Visual:     3 surfaces × {en LTR, ar RTL} × {mobile, desktop} rendered in `dark` →
            ...
            (adversarial, ux-reviewer) 14/14 ✓ — the filter bar's library controls
            (this repo: PrimeVue Dropdown + Calendar; an MUI / Vuetify / Ant / shadcn
            app reads the same way) re-themed via explicit :deep() overrides ·
            Chart.js series re-themed to the dark palette in the chart's own config
