...
- commands: design-review is the read-only audit; enhance-ui is the orchestrator command for full UI/UX enhancement (cleanup → iterate → re-enforce). ui-crawl + ui-crawl-fix (the Playwright cross-route crawler + auto-fix loop, v1.2+) are kept out of minimal because they require Playwright + axe install plus a `tests/crawl/` scaffold; they ship in standard mode.
- skills: ui-design-sweep is the closed 18-verb closure-vocabulary the frontend half of /polish + /ui-sweep + /enhance-ui dispatch; design-token-audit / motion-audit / a11y-quick-check are short detector audits; design-iterate generates 3 visual variants for /enhance-ui to dispatch.
