...
- **`evolve` (default)** — the existing visual world is the canon to HONOR and AMPLIFY. Infer the product's *implicit* concept from the current design, name where it is timid/generic/off its own concept (redlines dominate), and propose how to RAISE THE CEILING without breaking recognizability: the three directions stay on-brand (same type DNA, color family, core shape language) and differ in which axis they push and how far. Latitude: add/retune tokens, extend primitives with variants, introduce ONE new signature moment — but do NOT replace the grid base unit, type CLASS, or core neutral model wholesale, and carry an alias/back-compat note. Output centers on a **Direction Delta** (current value → proposed value per token/primitive) and hands off low-churn. Even bounded, the three MUST pass the divergence check and at least one MUST make a genuinely loud move. **The before→after glance gate (evolve-specific):** the recommended direction's rendered key surface MUST be visibly distinguishable from the CURRENT surface at a 1/8 thumbnail — a stranger shown both thumbnails must be able to say which is new. If they read as the same screen with retinted chrome (a new accent, a sparkline added, secondary items collapsed — but the same silhouette, density, and layout rhythm), evolve has degraded into a **restyle** → HALT to `/enhance-ui`·`/polish`. Amplifying the ceiling means the eye notices at a glance, not just on inspection; "same layout, new paint" is the exact failure this gate exists to catch. This gate is enforced operationally by `/art-direct` before the build, not merely asserted here.
