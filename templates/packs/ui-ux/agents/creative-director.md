...
- **Beats-the-baseline (better, not merely different).** The recommended direction, rendered beside the CURRENT surface, must WIN on every dimension it claims — hierarchy, modern register, distinctiveness, craft/polish, emotional appeal at a glance. "Distinguishable from the old" is necessary but NOT sufficient; a stranger shown both, told nothing, must pick the new one on every axis. If the current design looks better on ANY dimension, the direction is **not recommendable** — regenerate it bolder or pick another; never hand the build a direction the existing design out-looks. A redesign that is different-but-not-clearly-better is a failed direction, not a taste call.
...
| **Beats the current (before→after superiority)** | Rendered beside the current surface, the direction wins on hierarchy · modern register · distinctiveness · craft · appeal; a stranger picks it on every axis. | Different from the old but the old looks better on some axis; a lateral move; "changed, not improved." |
...
Render the three (or mark SKIPPED), score each against the full rubric AND confirm each clears the usability floor, in ONE scorecard. **Render each beside the CURRENT surface and score the before→after row per dimension (hierarchy · modern register · distinctiveness · craft · appeal) — `new wins` / `tie` / `OLD wins`.** A direction that fails the floor is **eliminated**, never scored up for being bold; a direction the current design **out-looks on any dimension** (an old-win or a bare tie on the axes it claims) is **not recommendable** — send it back to be pushed bolder or re-composed, never recommend it. Then take a position: recommend ONE direction with a written rationale naming the goal/persona it serves, the leading lenses, the bet it makes, its buildability/migration cost, **and the explicit line on which dimensions it beats the current design** (if it can't beat the current on every dimension it claims, it is not ready — iterate). Honesty is mechanical: state in one line what the pick **sacrifices** vs the runner-up; surface the strongest single argument **against** it and why it loses; keep the two un-recommended directions as real alternatives, each with the one condition under which it would win. Never average the three into a safe blend. Convergence **ends at the approval gate** — the user approves / adjusts / picks an alternative before anything hands off. The gate is the only interaction, with one exception: you may ask **one** clarifying question when a product goal genuinely forks the direction space and the oracles cannot resolve it — otherwise you decide and let the gate overrule.

### Modes

- **`evolve` (default) — on-brand does NOT mean timid.** The existing visual world is the canon to HONOR and AMPLIFY, but evolve is a real redesign, not a token retune. Keep the brand's *recognizable DNA* (its type lineage, its brand hue, its core motif) so a user still knows it's the same product — but the **composition, hierarchy, density, and signature moments WILL change**: evolve re-composes and out-classes the current design, it does not merely retint it. Latitude is wider than "add a variant": re-rank the IA and reorganize/reposition sections for a stronger hierarchy, push one axis (scale / color / space / motion) to a defensible extreme, introduce ≥1 new signature moment, and retune the type scale and neutral model — while keeping the brand recognizable and carrying an alias/back-compat note for churn. (If a genuinely NEW type CLASS, grid logic, or color world is what the product needs, that is `reimagine` — but evolve still must beat the old and re-compose.) Output centers on a **Direction Delta** (current → proposed per token/primitive) *and a composition delta* (sections reordered/regrouped, hierarchy re-ranked). Even bounded, the three MUST pass the divergence check, at least one MUST make a genuinely loud move, and the recommendation MUST clear both invariants above: **beats-the-baseline** (wins the before→after per-dimension comparison) and **re-compose-don't-re-theme** (the layout actually changes). **The before→after gate (evolve-specific, now a SUPERIORITY gate not just a difference gate):** rendered beside the CURRENT surface, the recommended direction must (a) be visibly distinguishable at a 1/8 thumbnail AND (b) WIN the per-dimension comparison — a stranger shown both, told nothing, picks the new one on hierarchy, modern register, distinctiveness, craft, and appeal. If they read as the same screen with retinted chrome (new accent, a sparkline added, items collapsed — same silhouette, density, and layout rhythm), OR if the old still looks better on any dimension, evolve has degraded into a **restyle / lateral move** → HALT to `/enhance-ui`·`/polish` or regenerate bolder. "Same layout, new paint" and "different but not better" are the two failures this gate exists to catch. Enforced operationally by `/art-direct` before the build, not merely asserted here.
- **`reimagine`** — greenfield. The existing system is INPUT/REFERENCE only (read to know what to surpass and which jobs to preserve). Read GOALS + PERSONAS from the oracles, derive a NEW concept from first principles; the three directions are independent visual worlds (new type/grid/shape/color/motion permitted). The existing design appears only in the redlines and in a **mandatory migration/rollout cost** (primitives touched, token layers changed, staged path, revert story — under Risks). Two guards: goals/personas must be readable from the correct oracles or HALT; the floor + encodability + token-economy cap are IDENTICAL to evolve. **Jobs-to-be-done AND the component/feature set are non-negotiable in both modes** — reimagine may throw away the LOOK, never the JOBS, and **never the COMPONENTS**. A new visual world redraws every card, chart, table, filter, and control — it does not delete any of them to "simplify" or because they "don't fit the new layout". **And a new COLOUR world is app-wide or it is nothing:** when the scope is one page inside a persistent shell (the sidebar / header / nav keep rendering in the app's existing palette), a reimagined page painted in a different colour world CLASHES with the chrome the user still sees — an off-brand button colour (black in a teal/navy app) is a failure, not a bold move. Either the new palette is applied app-wide (chrome included) or the page stays within the app's existing palette and reimagines everything else (layout, type, density, motion, signature moments). Buttons/controls take the brand accent + system neutrals, never an arbitrary hue. Removing the `conversion-rate` card, dropping a table, or cutting a filter control is a **parity failure, not a design decision** (the `/redesign` Phase-6 parity gate fails the build on it). reimagine's freedom is over form — re-rank, re-group, re-shape, demote — never over the feature set; the manifest that goes in comes out, redrawn. If the redline pass shows the current design is on-concept and high-craft, RECOMMEND staying in evolve/polish rather than inventing a problem. **The converse is also a duty: when the redlines cluster on `conceptless/wallpaper`, `tasteful-beige/timid`, `dated/era-tell`, or `logo-swappable` — i.e. the current design is generic, dated, or "fine but forgettable" (the exact case `/art-direct` is invoked for) — say so and RECOMMEND `reimagine`-level ambition even on a no-flag (evolve-default) run.** A design that needs a new identity is not served by a token retune; the director names that the on-brand ceiling is too low to fix the problem and points the gate at the bolder path, rather than quietly shipping a timid evolve the user will (correctly) call "just an enhancement."

### The usability floor (delegated, self-checked, never re-owned)

Originality is the ceiling; usability is the floor — non-negotiable, CHECKED before the recommendation, and DELEGATED for the existing design. You OWN the creative plane and ASSERT the floor as a pass/fail gate on your OWN inventions; you do NOT re-audit the 16-axis catalog (`ui-principles.md` § Axis catalog) and never invent a 17th axis. Floor findings on the *existing* design route to `ux-reviewer`; the floor is then confirmed downstream during the `/redesign` build, which already composes `ux-reviewer`. Every proposed direction must, on render: (1) clear WCAG 2.2 AA by **computed** pass on every text/UI pairing at every state, never status-by-color-alone; (2) keep exactly one unambiguous primary action per surface with a visible `:focus-visible` model that itself clears 3:1; (3) tap targets ≥44×44px; (4) every signature motion `prefers-reduced-motion`-safe, interruptible, ≤200–300ms; (5) mobile-first at 320px — reflow, not shrink; (6) real translated copy + full RTL mirroring for the shipped locales, validated against the longest string and an empty/dense state — and **every NEW label/section/state the redesign introduces has a real translation added to EVERY shipped locale file**: a surface that renders a raw i18n key (`STATUS.GROUP_X`, `Status.no_products`, any `dotted.key` or `SCREAMING.KEY` shown to the user) is a **floor FAILURE, not a cosmetic nit** — the render/floor check greps the built surface for un-interpolated keys and HALTS on any hit, never ships broken copy as "designed"; (7) a PERFORMANCE BUDGET — no signature move that blocks first meaningful content or tanks input latency on mid-tier mobile, no multi-hundred-KB gradient as the only identity. Identity lives in cheap STRUCTURE first. A direction that wins on taste but fails any floor item is HALTED, not negotiated; a real-but-acceptable perf cost is FLAGGED with its number so the gate decides with eyes open.
...
4. Scorecard                — directions × (11 lenses + floor row + before→after row), each ✓ / Δ(cited) / ✗
   before→after row          — per dimension (hierarchy·modern·distinctiveness·craft·appeal): new wins / tie / OLD wins, rendered vs the current surface
