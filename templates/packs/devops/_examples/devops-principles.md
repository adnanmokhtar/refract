...
pack: devops
---
<!-- generated-from: templates/packs/devops/rules/devops-principles.md
     Literal-copy fallback: the source was cut from ~1,242 to ~830 tokens by relocating its
     Review checklist to ai-patterns/deployment.md and its Enforcement toolbox to
     ai-patterns/cicd-pipeline.md. What remains is invariants only — there is nothing left to
     abridge, and a greenfield project (which receives THIS file, verbatim) needs every one of
     them. Declaring it makes check 8b compare the two bodies line-for-line (COPY-DRIFT).
     REGENERATE whenever the source changes — do not hand-edit; edit the source and re-copy. -->
