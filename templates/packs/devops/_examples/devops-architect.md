...
Two agents holding two numbers for the same question is how a project ends up with two answers.
This table is the standalone fallback, and it asks about **capability**, not headcount: "10
engineers" is not what makes Kubernetes work, and a 4-person team with a platform engineer will
operate it better than a 20-person team without one.
...
| Fly.io / Railway / Render | Push-to-deploy; nobody's job includes running infrastructure | Heavy compliance, or regions/controls beyond the platform's set |
...
| Bare K8s | Dedicated platform/SRE capacity, and the managed control-plane markup is material at your scale | Anyone else |

**The question that decides it** is not team size but: *what does this platform cost you on a Tuesday
when nothing is broken?* Managed K8s bills a control plane, a recurring upgrade, and a class of
failures (scheduling, networking, admission) whose debugging is a skill. If nobody has that skill and
nobody is being hired for it, every row above K8s is cheaper.
...
- **Builder and runtime share a libc unless the artifacts are provably portable.** Alpine is musl,
  `slim`/`bookworm` are glibc; native addons, compiled wheels, CGO binaries and generated ORM engines
  built against one will not load on the other. Most common "builds fine, dies on start" cause, and
  it is a design choice — the libc constrains every future native dependency.
...
- `.dockerignore` excludes `.git`, `node_modules`, `tests/`, dev configs, README. Size is judged
  against the project's recorded baseline, not an absolute. What is always a defect is builder layers
  surviving into the runtime stage; `docker history` shows it.
...
- Matrix only across versions that are **both** supported in prod and still supported upstream.
  ...
  from memory — a leg on an EOL runtime certifies something unpatched. Single-version is usually right.
...
- Image scan: Trivy / Grype / Snyk fails on HIGH+ CVE, and runs **before** the push — scanning a
  published image gates the deploy, not the artifact.
...
- `/dockerize` Phase 4 — for the base-image and runtime-user decision (including the libc question
  above, which that command's Phase 4 depends on).
...
- `@infra-architect` (infrastructure pack) — **owns the platform-tier decision**. Defer to it and
  design within the tier it returns; the matrix above is the devops-only fallback.
