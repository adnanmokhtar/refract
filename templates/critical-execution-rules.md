...
2. ✅ **Knowledge extracted** — every existing setup file (CLAUDE.md, AGENTS.md, ai/**/*.md, .claude/rules/*.md, .claude/agents/*.md, .claude/skills/**/*.md, .claude/commands/*.md, .claude/codebase-profile.md) is READ and the durable insights pulled into `.claude/_refresh-extract.md` — the file `scripts/refresh-extract-checklist.sh` already scaffolded in Phase 0.0. Phase 0.2 FILLS its `<TBD>` sections; it does not create a second file, and the file is NOT deleted afterwards (`audit-setup.sh` C2b errors when it is absent). Sections: ADRs + append-only history, validated corrections, project intent + business glossary, custom rules + conventions, custom agents/skills/commands, implicit architecture, detected stack, migration mapping, hook deltas, catch-all sweep, drop list. Without this, REFRESH discards accumulated knowledge — the failure mode that motivated this mode in the first place.
