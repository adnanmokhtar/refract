...
- **Gitignore the learning-hook + scheduler outputs** (CRITICAL — prevents a dirty-tree commit loop). The hooks just wired (`post-commit-learn.sh`, `post-merge-learn.sh`, `update-session-log.sh`) and the scheduler APPEND to files on every commit / merge / session-stop. If those files are tracked, the working tree is dirty the instant a commit finishes — you can never reach a clean tree (commit → post-commit hook rewrites the file → dirty → commit again → hook fires again → …). This bit real projects (capsolah-api, 2026-06). Ensure `.gitignore` contains these entries (idempotent append; create `.gitignore` if missing; skip any entry already present). For each, also `git rm --cached --quiet <file>` if it is already tracked, so the loop stops immediately:
  ```gitignore
  ...
  ```
  Rationale: these are local learning/session state, not shared history — same treatment as `.claude/plans/` and `.claude/_telemetry.jsonl`. A team that deliberately wants a committed changelog must instead move the post-commit append to a pre-commit/staged step (a post-commit write to a tracked file is always a dirty-tree loop); record that decision in the ledger.
