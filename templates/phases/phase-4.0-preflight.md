...
- **Warm the import graph** — **`run-preflight.sh` step 5/5 already did this**; nothing is left for you to run. It was an instruction here first, and being an instruction is why it did not happen: three real setups finished green with no `.claude/_graph.json`, so `inject-blast-radius.sh` `exit 0`-ed on every edit for the life of those projects. Your job is to READ what step 5/5 printed and carry it into the setup summary: the node/edge counts, any `NOT READ:` disclosure, and the `0 nodes` note if it appeared. **0 nodes on a repo that has sources is a finding, not a blank** — it usually means the aliases live in a JS config (`vite.config.ts`, `webpack.config.js`) the resolver refuses to execute, or the stack is outside TS/JS, Vue/Svelte, Python and Dart. Say which in the summary. To rebuild by hand later: `python3 ~/.claude/scripts/build-graph.py --corpus=project --repo=. --stats`.
