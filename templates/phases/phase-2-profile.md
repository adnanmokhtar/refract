...
members:                        # never empty — `single` carries exactly one entry, root `.`
  - name: <member-a>
    ...
    manifest: <path/to/member-a/<manifest-file>>   # or `(none)` — a member dir with no manifest is still a member
```

**The receiver, named.** All three are declared inputs of the orchestrator skill — `extract-codebase-overview/SKILL.md § Inputs` marks `repo_shape`, `shape_signal` and `members` **required** — and its Step 2 consumes them: the `## Repository shape` entry set IS `members` (same order, one entry each, no additions and no drops), a manifest scan is explicitly barred from re-answering the shape question there, and an absent or empty `members` halts with `[SHAPE-INPUT-MISSING]` instead of falling back to a `single` default. Step 2.5 buckets the census on those entries and halts when it has fewer buckets than members. A handed-in value nothing branches on is decoration; this one is checkable in the skill file, and **every step in that skill's per-section denominator table runs per member over that member's bucket — Steps 3, 4, 5, 6, 7, 8 and 12** (`extract-codebase-overview/SKILL.md § Step 2.5`, the row per section, and the sentence under it: *"Every population in that table is per member"*). Take that list from the table rather than restating it from memory — Step 5 (base classes → the idiom substrate every Phase 4 generator reads) is capped by `parallelism` and is in the set, and Step 7 (API surface) is in it at a different setting: the skill declares **no cap** for Step 7, so its denominator row reads *"none declared → must be 100%"* and its per-member population is walked in full rather than sampled. Note what enforces that, because the answer CHANGED and both positions are on the record: the 100% obligation is written in the Step 2.5 table cell, and **check 7 now enforces it too**. Its cap-cell bullet FAILs a row whose `Cap` reads `none declared` when `seen < present`, *disclosed or not* — so a run that walks 12 of 40 controllers and writes `[SAMPLED: 12/40 files]` on `## API surface` is now red, where it used to pass. Disclosure is still required; it is no longer sufficient. Read `extract-codebase-overview/SKILL.md` § Step 15 — its `Coverage sweep (check 7)` bullet list is where the `none declared` row now FAILs — and § Step 7 for the current rule. The learning pack's `CHANGELOG.md` **1.4.1** entry describes the OLD state — check 7 asserting nothing about the uncapped row — and is correct as history; **1.5.0** records the bullet being added. Cite 1.5.0, not 1.4.1, for what the check does today.
...
3. **Every capped or sampled step runs once per member, over that member's population** — architecture layers, modules, idioms, data model, API surface, conventions, recent activity. That is the same seven-step set as the skill's denominator table, in section order; API surface is the one with no cap, which makes it 100% per member rather than exempt. A step that walks the union and reports one winner produces the blended value § 17 forbids, and produces it with a citation that resolves, which is what lets it survive review.
