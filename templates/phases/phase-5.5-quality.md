...
**Rule**: while `_extracted-codebase.md` carries any `[SAMPLED: <seen>/<present> <unit>]` section marker, `PLATEAU-DEEP` may not be emitted. The verdict becomes `PLATEAU-WEAK` with reason `coverage`.
