...
- **Confirm the extraction actually split on members.** When the repo-shape block of `.claude/codebase-profile.md` (found BY KEY `members:`, not by section number) lists ≥ 2 members, `.claude/_extracted-codebase.md` must carry one `## Repository shape` entry AND one census bucket per member, and every per-member field (1–10, 15) must name the member it came from. One entry or one bucket where `members:` has two means Phase 2 ran member-blind (`phase-2-profile.md § 2.0.a`) — the per-member values were sampled repo-wide, so they are a blend, not a measurement. ERR, not a warning: this is the failure mode that costs nothing to detect here and cannot be detected at all once the artifacts are written from it.
