...
- ENHANCE / REFRESH: **the mode does NOT decide whether to populate — the stub-vs-authored test does.** A foundational file (`ai/{architecture,stack,modules,status,conventions,business-domain,_convention-cheatsheet}.md`) is a **scaffolded stub** if it is absent, OR byte-identical to its `templates/repo-baseline/ai/<f>.md` source, OR still contains stub-only placeholder tokens (`<src/path`, `<e.g.,`, `<YYYY-MM-DD>`, `<detected`, `<EntityA>`, `<DetectedBase>` — NOT bare `<name>`, which is a legitimate code/path placeholder in populated docs). **A scaffolded stub MUST be populated from extraction exactly as in CREATE** — the baseline-copy step just dropped it in this same run, so there is no user content to protect. Only a file that *diverges* from the baseline stub is genuinely user-authored: leave those untouched (prepend a dated `Recent Changes` entry to `ai/status.md` only). Add new files (ADRs, patterns, business-domain content) ONLY if a new concept emerged; never rewrite divergent user content.
  > **The bug this closes**: a first-ever setup on an existing code repo runs in ENHANCE, the baseline scaffold copies the stubs into `ai/`, and the old "leave user-authored files untouched" rule then preserved those freshly-copied stubs forever — because it could not distinguish a 2-second-old stub from real user content. Keying on the baseline-stub test, not the mode, fixes it.

**Tiered context loading (NOT mandatory pre-Phase-4 deliverables):** the 6 foundational files (`CLAUDE.md`, `ai/conventions.md`, `ai/business-domain.md`, `ai/core/glossary.md`, `ai/project-goals.md`, `ai/users-and-personas.md`) are **Tier 1 context** — loaded on demand by agents that need them. They are NOT a doc-before-code gate that blocks Phase 4 from emitting code-touching output. CREATE-mode commands (which establish these from scratch) DO populate them in Phase 5 (Update). ENHANCE / REFRESH commands populate them when present-but-stub or absent (the stub-vs-authored test above) and consult them only when present-and-user-authored — a scaffolded stub is NOT "user content to preserve."
**Validator:** Phase 5.3.6 verifies, in **every mode**: every foundational `ai/` file is present AND non-stub (not byte-identical to baseline, zero placeholder tokens). User-authored divergence is preserved; a surviving stub is a HALT, not a pass. (The prior "ENHANCE/REFRESH → no recreation of user-authored versions" rule wrongly green-lit un-populated stubs — that is the exact failure this gate now catches.)

**`ai/conventions.md` MUST be auto-populated from the codebase profile** (not generic). Generic conventions ("use camelCase") are useless. The file content is built from Phase 2 detection results — every value below is a placeholder filled at write time from the **deep-extraction source**. The LLM authoring this file MUST NOT carry concrete class names / paths / library names from any other project's run.

> **Deep-extraction source resolution (Fix C — name drift).** Phase 0 writes `.claude/_codebase-scan.md`; Phase 2 writes `.claude/codebase-profile.md` (condensed) and, in deep modes, `.claude/_extracted-codebase.md`. These names have drifted across the templates, and a run that produced `_codebase-scan.md` but not `_extracted-codebase.md` previously caused population to silently no-op. The populator (and every `ai/` template's "auto-populated from …" line) MUST resolve the source in this order and use the **first that exists**: `.claude/_extracted-codebase.md` → `.claude/_codebase-scan.md` → `.claude/codebase-profile.md`. Population MUST NOT skip because the canonical name is absent — fall through the resolution order. If none exists, that is a HALT (extraction never ran), not a silent skip.

**`ai/runtime/*` — seed the DERIVABLE files at setup; never fabricate the DISCOVERED ones.** `ai/runtime/*` are baseline-scaffolded but were historically never populated (same root cause as the foundational-stub bug). They split by source, and the stub-vs-authored test above applies per file:
- **`ai/runtime/domain-anti-patterns.md` — SEED (deterministic).** When Phase 2 detected a business domain, copy/adapt `~/.claude/templates/business-domains/<detected-domain>/anti-patterns.md` into it. The file's own header documents this ("Populated at `/setup-project` from `business-domains/<detected>/anti-patterns.md`"); leaving it as the generic stub when a domain pack exists is a bug.
- **`ai/runtime/dependencies-with-traps.md` — SEED the env-var inventory.** From `.env.example` / `.env*`, list every env var with required-vs-optional + default + what-breaks-when-missing (derivable). Leave the per-package `Trap:` entries empty — those are discovered.
- **`ai/runtime/context.md` and `ai/runtime/environment-quirks.md` — DO NOT auto-fill.** These hold runtime gotchas discovered while working (event ordering, cold-starts, out-of-band migrations). Auto-generating them risks **fabricating** quirks that don't exist — worse than an empty template. Leave the template (with its `<e.g., …>` illustrations). They are correctly excluded from the §5.3.6 hard gate for this reason.
**`ai/core/invariants.md` — POPULATE with project-specific invariants (don't ship the generic stub).** The baseline `ai/core/invariants.md` lists *generic* invariants (multi-tenant `tenantId`, money-as-minor-units, soft-delete exclusion, idempotency). `fix-bug.md` reads this file to know "violating one of these is a bug, not a style issue" — a generic stub gives it nothing project-specific to check against. Populate it from extraction, keeping only the generic invariants the project actually exhibits and adding the project's own:

- **Source order**: `.claude/_refine-extract.md § Domain entities — <domain>` (the `invariants:` blocks per entity — statement + `citation: <file:line>` + `enforcement:` — written by `extract-domain-entities-deeply` in `--refine` mode) → `.claude/_extracted-codebase.md § Data model` + `§ Cross-cutting concerns` (round-one: confirmed multi-tenant / soft-delete / audit-column / money-column signals) → first that exists. If neither exists, leave the generic stub (CREATE mode, no code yet).
- **What to write**: for each confirmed cross-cutting concern, keep the matching generic invariant and make it concrete (cite the real column / base class / middleware from extraction, e.g. `tenant_id` on `BaseEntity` at `<path>`). For each entity with an extracted `invariants:` entry, add a project-specific line under the relevant heading (Data integrity / Concurrency / etc.). **Drop** generic invariants the project does NOT exhibit (no multi-tenant signal → remove the tenant-isolation block rather than asserting a constraint that isn't true — a false invariant is worse than an absent one).
- **Provenance + safety**: every concrete invariant cites a real file from extraction (no invention — same leak-scan discipline as `ai/conventions.md`). The stub-vs-authored test applies: a user-diverged `invariants.md` is left untouched; a scaffolded stub is populated. Phase 5.3 treats a surviving generic-only stub on a project WITH confirmed cross-cutting signals as a miss to report.

- **Link the seeded runtime files from the entry docs.** A seeded file nothing points at is orphaned in the cross-reference graph — agents (in Claude AND every adapter that reads `AGENTS.md` / `.cursor/rules/00-project`) never discover it. `CLAUDE.md`'s "Where things live" MUST carry an `ai/` knowledge-layer pointer that names `ai/runtime/dependencies-with-traps.md` + `ai/runtime/domain-anti-patterns.md` (the codex adapter then propagates it to `AGENTS.md`; the cursor adapter to `00-project.mdc`). `ai/stack.md`'s Env-files section MUST cross-link `ai/runtime/dependencies-with-traps.md`. Use full `ai/runtime/...` paths (not bare `runtime/...`) so the references resolve and are grep-discoverable.
...
## Before changing a shared file — ask the graph, don't grep

```bash
python3 ~/.claude/scripts/build-graph.py --corpus=project --repo=. --who-breaks <path>
python3 ~/.claude/scripts/build-graph.py --corpus=project --repo=. --central   # load-bearing files
```
