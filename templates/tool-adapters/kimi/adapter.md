...
**Capabilities:** R (rules via skills + AGENTS.md fallback), S (native skills), H (native hooks via TOML), A (custom subagents via YAML), C (action commands via dual-surface: skill + subagent, each with the EXECUTE NOW preamble — see § "Skills vs subagents — which primitive for what"; OPTIONAL Flow-Skill upgrade for `/flow:<name>`) — no user-extensible slash-command surface, no path-scoped rules.

> **Brand-group sibling.** Kimi shares the "brand directory" pattern with Claude Code and Codex: per the Skills doc, "Brand group (mutually exclusive): `~/.kimi/skills/`, `~/.claude/skills/`, or `~/.codex/skills/`." Project-level mirrors that: `.kimi/skills/`, `.claude/skills/`, `.codex/skills/`. Kimi's primary user surface is **skills + subagents** — there is no separate `/<command>` surface. **Custom subagents replace what Claude Code calls slash-command-driven agents** — and this distinction is load-bearing: subagents EXECUTE when invoked, skills LOAD as reference. The adapter MUST route `commands/*.md` (action-style workflows like `/optimize`, `/migrate`, `/polish`) to **BOTH** surfaces — a skill (so `/skill:<name>` works) carrying an EXECUTE NOW preamble that makes the loaded text imperative, AND a subagent (so the main agent can dispatch by description) — never to a skill alone with no preamble. See § "Skills vs subagents — which primitive for what".
...
|---|---|---|---|
| `commands/*.md` action commands (`/optimize`, `/migrate`, `/polish`, etc.) | **Action — user invokes, work happens** | **Dual-surface**: **Regular Skill** at `.kimi/skills/<name>/SKILL.md` (with an "EXECUTE NOW" preamble) **AND** a **Subagent** at `.kimi/subagents/<name>.yaml` (same preamble + tool whitelist) | `/skill:<name>` loads the skill body as prompt — the EXECUTE NOW preamble makes the AI treat it as a directive (Kimi's docs call loading "contextual guidance, not strict", so execution is best-effort but the preamble works in practice). The subagent adds a description-dispatch surface ("run optimize on src/") with a tool whitelist. This is what every pipeline surface emits. See § "Skills vs subagents — which primitive for what". |
| `commands/*.md` action commands — **OPTIONAL upgrade** when a Flow diagram is worth authoring | Same | **Flow Skill** at `.kimi/skills/<name>/SKILL.md` with `type: flow` + Mermaid/D2 diagram | `/flow:<name>` autonomously walks BEGIN→END — Kimi's only built-in primitive with **explicit execute-without-asking** semantics. This is a *stronger* execution guarantee than the regular-skill preamble, but authoring the diagram is costly and the adapter does NOT generate it by default; the dual-surface (skill + subagent) is the shipped baseline. Promote a command to a Flow Skill only when the extra authoring is justified. |
| `skills/<name>/SKILL.md` reference procedures (`extract-v1-contract`, `parity-test-generate`, `perf-uplift-survey`) | **Read when relevant** | **Regular Skill** at `.kimi/skills/<name>/SKILL.md`, no preamble | These ARE reference. Skills auto-discover; AI consults SKILL.md when description matches user's task. |
...
1. **Shipped baseline (dual-surface)**: Regular Skill with EXECUTE NOW preamble (user invokes `/skill:<name>` → usually executes, contextual guidance) **AND** a Subagent + parent-agent registration (main agent dispatches via description match). This is what the adapter generates for every command — two invocation surfaces, no diagram authoring required.
2. **Optional upgrade**: Flow Skill at `.kimi/skills/<name>/` with `type: flow` + Mermaid diagram → user invokes `/flow:<name>` → autonomous execution. Strongest execution guarantee, but the adapter does NOT generate the diagram by default — promote a command to a Flow Skill only when the extra authoring is justified.
...
```toml
# Recommended hooks for projects using Refract (paste into ~/.kimi/config.toml)
[[hooks]]
event = "PreToolUse"
matcher = ".*"
...
- **`/upgrade-dep` (dependency / framework / runtime upgrade, any ecosystem, 2026-09):** one bounded version span under ONE app — not a V1→V2 port (`/migrate`) and not a staleness report (`/dependency-vuln-check`). A translation MUST preserve four things or it is not this command: (1) **cite-or-halt on the vendor's breaking-change source** for the exact span — a major upgraded from recollection is the defect the halt exists for; (2) the **parity oracle is pinned and its baseline captured BEFORE the first edit**, chosen by `PROJECT_KIND` (suite + boot + route inventory + contract diff for `backend-*`; typecheck + build + route-mount sweep for `frontend-*` / `mobile-web`; **both** platform targets building for `mobile-rn` / `mobile-native` (Flutter and the native toolchains alike); exported-symbol diff for a library) — a project with no oracle gets the cheapest one committed first, never a silent pass; (3) **staged commits, never mixed** — manifest+lock, then the codemod's output verbatim and alone, then one commit per breaking-change item, then config re-shaping, so `git revert <codemod-sha>` stays a real rollback; (4) **majors one at a time** and, for a runtime-class upgrade, every in-repo pin (version file, manifest constraint, Dockerfile `FROM`, CI matrix, deploy runtime) moves in the same run or the run halts — a pin outside the repo is named under `Risks:`, never dropped. No ledger, no `progress.md`: it is one-shot. The run closes with the mandatory `Not validated:` / `Risks:` / `Revert:` block plus paste-ready next steps; **no `validate-*-artifacts.sh` ships for it** — the contract is carried by the command's own halt conditions, so an adapter must not claim a script gate it does not have. Surface it through the same native path this adapter already uses for `/migrate`. The run is shell-heavy (package manager, codemod, oracle); where this tool gates shell execution, document that seam rather than reporting an upgrade that never ran. See `commands/upgrade-dep.md` + `templates/tool-adapters/_orchestration-sync.md`.
...
- **`/delegate` (cross-tool dispatch, CLI-backed):** hand ONE bounded task to a **different** AI coding CLI running as a separate process against this tree, then review the diff and commit it yourself — the relay never commits. This tool surfaces it dual-surface like every other action command: `.kimi/skills/delegate/SKILL.md` with the EXECUTE NOW preamble (a regular skill loads as "contextual guidance, not strict") **and** `.kimi/subagents/delegate.yaml` whose `tools:` whitelist **must include `shell`**, registered in the parent agent YAML or it is an orphan Kimi cannot dispatch. Shell access is not the constraint here — invocation reliability is, which is why the OPTIONAL Flow-Skill upgrade is the only surface with explicit execute-without-asking semantics. As an **implementer** it is `kimi`, the one target where **`--read-only` is `unenforceable`**: Kimi documents no read-only mode at all (`P_RO_CLASS="none"`), so G3 refuses at exit 5 unless the human passes `--accept-unenforced` and accepts an unenforced run. Resume relay-unsupported. Canonical recipe + per-tool matrix: `templates/tool-adapters/_delegate-integration-coverage.md`; contract: `commands/delegate.md`; relay: `scripts/delegate-relay.sh`.
- **Project-memory recall (`/recall` + `recall-inject.sh`):** BM25 retrieval over this project's existing `ai/` memory — the `ai/dynamic/` sinks, the don't-retry catalog at `ai/failures/_index.md`, promoted ADRs / patterns / conventions, and the archives. Adds no sink and stores nothing; every row is a **POINTER** (`path:line`), never a paraphrase. Two halves that rank tools differently: the `/recall` command a human invokes, and the opt-in (`touch .claude/.recall`) prompt-time hook that surfaces the same memory unasked. **Plain stdout, not JSON — and registration is user-global.** Kimi's `UserPromptSubmit` contract is *exit 0 with non-empty stdout appended to context*, so the shim MUST drop the script's `jq -cn '{hookSpecificOutput:…}'` envelope and print the context bare; emitting Claude's JSON injects visible JSON into the conversation. Because `[[hooks]]` live in user-global `~/.kimi/config.toml` and not per-project, the hook fires in **every** repo that user opens and the script's own `[ -f ".claude/.recall" ]` + `[ -d "ai" ]` guards are the only thing keeping it inert elsewhere — the marker file is load-bearing here in a way it is not on any other tool, and this adapter must state that, not bury it. Query half is dual-surface like every action command: `.kimi/skills/recall/SKILL.md` with the EXECUTE NOW preamble plus `.kimi/subagents/recall.yaml` whose `tools:` whitelist must include `shell`, registered in a parent agent YAML or it is an orphan. Canonical recipe + per-tool matrix: `templates/tool-adapters/_memory-recall-coverage.md`; command: `templates/packs/learning/commands/recall.md`; hook: `templates/repo-baseline/.claude/hooks/recall-inject.sh`; engine: `scripts/pack-search.py --catalog=memory` + `scripts/gen-memory-catalog.py` (**install the pair into `~/.claude/scripts/` — same directory, or the hook silently no-ops**).
