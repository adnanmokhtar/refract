# OpenCode adapter

**Tool:** SST's OpenCode (open-source Claude Code alternative).
**Docs:** https://opencode.ai/docs/config/ · https://opencode.ai/docs/agents/ · https://opencode.ai/docs/commands/ · https://opencode.ai/docs/skills/
**Capabilities:** R, A (native folder), S (native folder), C (native folder), **H (partial — native plugin event handlers at `.opencode/plugins/*.ts`: `tool.execute.before`/`.after`, session/file/permission events; JS/TS, not shell-command config)**.

> **OpenCode standalone goal**: a project generated by `/setup-project` with the `opencode` adapter selected MUST work end-to-end in OpenCode with **no `.claude/` present**. Agents, commands, and skills land in OpenCode's native `.opencode/{agents,commands,skills}/` folders so they appear in OpenCode's pickers exactly the way Claude Code surfaces them.

## Target files

```
repo-root/
├── AGENTS.md                                  # Cross-tool rules file (also read by OpenCode)
├── opencode.json                              # Provider config + instructions globs (+ optional commands fallback block)
└── .opencode/                                 # NATIVE — OpenCode reads this directory tree
    ├── agents/
    │   └── <agent-name>.md                    # NATIVE — one file per agent persona
    ├── commands/
    │   └── <command-name>.md                  # NATIVE — one file per slash command
    └── skills/
        └── <skill-name>/
            ├── SKILL.md                       # Skill body + frontmatter
            └── *.sh / *.py                    # Optional supporting scripts (copied verbatim)
```

OpenCode also continues to globally read `~/.config/opencode/` (user-level) — repo-local `.opencode/` overrides global.

> **Migration note**: pre-Apr-2026 setup-project crammed every command into the `opencode.json` `commands` block and relied on OpenCode globally reading `~/.claude/skills/`. The new layout uses repo-local native folders so the project is self-contained — no symlink trick required for OpenCode to see the project's skills.

## File formats

### `AGENTS.md` (shared with other AGENTS.md consumers)
Plain markdown. Same file that Codex, Cursor, Amp, Cline, Copilot, and Windsurf read. Treat as the single canonical instructions source.

### `opencode.json`
JSON. Schema (per OpenCode's published config schema as of Apr 2026 — only these top-level keys are accepted):
```json
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "anthropic": { "api_key": "..." },
    "openai":    { "api_key": "..." },
    "moonshot":  { "api_key": "...", "base_url": "https://api.moonshot.ai/anthropic" }
  },
  "model": "anthropic/claude-sonnet-5",
  "instructions": ["AGENTS.md", "CLAUDE.md", ".claude/rules/*.md", "ai/patterns/*.md"]
}
```

> **DO NOT** write `_generator`, `_generator_fields`, `_version`, `_generated_at`, `_note`, or `commands` keys at the top level. OpenCode's schema rejects unrecognized keys with `Configuration is invalid` and the binary refuses to start. Commands ship under `.opencode/commands/<name>.md` (see "Full artifact translation" below), not inside `opencode.json`.

The `instructions` array is an **extension point** — any markdown listed here is loaded as context. This lets OpenCode reuse our `.claude/rules/*.md` verbatim.

## Translation recipe

1. If `AGENTS.md` doesn't exist → write it (source: Claude Code adapter's `CLAUDE.md` compacted). If it exists → verify the discipline-enforcement block is present (between `<!-- discipline-enforcement:start -->` / `<!-- discipline-enforcement:end -->` markers); if missing, inject the block from `templates/tool-adapters/_discipline-enforcement.md` near the top of the file (after Project overview, before Architecture). **MANDATORY when migration / align / optimize / polish / per-pack-audit packs are loaded**. Without the block, OpenCode invents non-canonical paths (e.g. `.claude/_v1-scan-inventory.md` instead of `ai/migration/scan-report.md`) — breaks validators + gates.
1a. **Sync the block on every refresh.** SHA-256 hash check on bytes outside the marker block; user-authored content preserved verbatim.
2. If `opencode.json` doesn't exist → write it with ONLY schema-valid keys:
   - `$schema` → `"https://opencode.ai/config.json"`.
   - `provider` block (defaults: anthropic; user fills API keys).
   - `instructions` glob pointing at `AGENTS.md` + `.claude/rules/*.md` + `ai/patterns/*.md` + `ai/references/*.md`.
   - No model pinning unless user specifies (lets OpenCode pick based on provider availability).
   - **No metadata keys, no inline commands block.**
3. Commands from `.claude/commands/*.md` → write to `.opencode/commands/<name>.md` (the NATIVE folder; see "Full artifact translation" below). Do NOT embed in `opencode.json`.
4. Validate before write: the file MUST parse as JSON AND have only keys from the allowed top-level set (`$schema, provider, model, instructions`). Any other key → drop and surface a warning.

## Idempotency

`opencode.json` is regenerated by re-scanning the `instructions` list. The `provider` block is preserved bit-identically (the user owns those keys for their API credentials).

Tracking metadata (generator name + version + timestamp) lives in a SIDECAR file, NOT inside `opencode.json`:

```
.opencode/_setup-project-meta.json
```

Schema (sidecar — not constrained by OpenCode):
```json
{
  "_generator": "claude-setup-project",
  "_version": "<setup-project version>",
  "_generated_at": "<ISO timestamp>",
  "_generator_fields_in_opencode_json": ["instructions"]
}
```

The sidecar is the single source of truth for "what setup-project owns" and "what the user added." Re-runs read the sidecar to preserve user-added keys and overwrite generator-owned ones.

## Known gotchas

- OpenCode does NOT have its own agents directory. "Agents" in OpenCode means the same AGENTS.md — there's no specialized-persona dispatch.
- OpenCode is reported to read `~/.claude/skills/` globally (not per-project) — verify against current OpenCode docs at https://opencode.ai/docs/rules/ before relying on it. Skills in this repo's `.claude/skills/` are NOT auto-available to OpenCode unless the user globally installs them (or symlinks).
- Hooks ARE expressible via the plugin system (`.opencode/plugins/*.ts` — `tool.execute.before` can block, `tool.execute.after` runs post-edit checks). See § Hooks. Not the `settings.json`+shell-command form Codex/Gemini use — plugins are JS/TS that shell out to the repo's `.claude/hooks/*.sh`.
- Config can live in `~/.config/opencode/opencode.json` globally; per-project `opencode.json` overrides globals.

## Sample output

`opencode.json` (only schema-valid keys; no metadata, no inline commands):
```json
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "anthropic": { "api_key": "${ANTHROPIC_API_KEY}" }
  },
  "instructions": [
    "AGENTS.md",
    "CLAUDE.md",
    ".claude/rules/*.md",
    "ai/patterns/*.md",
    "ai/references/*.md"
  ]
}
```

`.opencode/_setup-project-meta.json` (sidecar; NOT constrained by OpenCode schema):
```json
{
  "_generator": "claude-setup-project",
  "_version": "3.0.0",
  "_generated_at": "2026-04-29",
  "_generator_fields_in_opencode_json": ["instructions"]
}
```

`AGENTS.md` (shared universal file — see codex adapter for the sample shape).

## Full artifact translation

### Commands — `.claude/commands/<name>.md` → `.opencode/commands/<name>.md` (NATIVE)

OpenCode reads `.opencode/commands/<name>.md` natively. The adapter copies each Claude command body to its native location verbatim; only the frontmatter is normalized — the load-bearing `agent:` field is added (see below) and the Claude-only orchestrator fields (`kind:`, `pack:`, `allowed-tools:`) are stripped, since OpenCode command frontmatter is `description:` + `agent:` (+ optional `model:`/`subtask:`) and those fields read as leaked Claude-isms:

```markdown
---
description: Hit a dev endpoint via curl and verify DTO shape
agent: build                          # REQUIRED for action commands — see frontmatter note below
model: anthropic/claude-sonnet-5   # optional — falls back to project default
---

# /endpoint-test

<body of .claude/commands/endpoint-test.md>

Arguments: $ARGUMENTS
```

**Frontmatter discipline (`agent:` field — load-bearing for execution semantics)**:

The `agent:` frontmatter routes the command to one of OpenCode's agent modes. For action commands that edit files / run shell / call MCP tools (`/optimize`, `/migrate`, `/polish`, `/align`, `/refactor`, etc.), `agent: build` is **REQUIRED** — Build mode is the only mode with full tool access. Without it, OpenCode routes the command to the default mode (typically Plan or Chat) which restricts tool calls. Symptom of the miss: user types `/optimize` and OpenCode responds "I would do X, Y, Z" instead of executing.

Per-command-class mapping for `agent:`:

| Command class | `agent:` value | Why |
|---|---|---|
| Audit-only (`/audit`, `/security-audit`, `/perf-audit`, `/db-audit`) | `plan` or a custom read-only subagent | Audits should not edit; Plan mode gives reasoning + read tools without write/shell |
| Plan-only (`/refine-prompt`, `/migration-plan`, `/draft-phase-adrs`) | `plan` (use `build` if executing `/refine-prompt`, which writes to `ai/prompts/` — or `ai/ideas/` for the `new-project` class) | Plan mode emits plans, not edits. **`/refine-prompt`'s `medium`/`heavy` deep-refine fans out to parallel specialist sub-agents in Claude Code; OpenCode has no specialized-persona dispatch, so the deep pass degrades to drafting the class-specific sections sequentially.** |
| Full-action (`/optimize`, `/migrate`, `/polish`, `/align`, `/refactor`, `/unify-surfaces`, `/setup-project`, `/do`) | `build` | Build mode = full tool access (read + write + shell + MCP) |
| Knowledge/help (`/setup-project-health`, `/learn-from-task`) | `build` (low-risk) | These write to `ai/` but don't shell out aggressively |

**This mapping is applied DETERMINISTICALLY by `apply-adapter-sync.sh`** (`opencode_classify_command_agent`) — no LLM authoring step and no `MISSING-AUTHOR` punt. Precedence: (1) an explicit `agent:` already in the `.claude` command source wins (author's declared choice — put one there to override); (2) `allowed-tools:` introspection — a pure-read toolset (no `Write`/`Edit`/`Bash`/…) → `plan`, anything that can write or shell → `build`; (3) conservative read-only NAME tokens (`audit`, `-scan`, `review`, `-lint`, `inspect`, `analyze`, `status`, `health`, `diff`, `trace`, `map`, `check-`/`-check`) → `plan`; (4) default → `build`. The asymmetry is deliberate: labeling an action command `plan` BREAKS it (Plan mode blocks its writes — the "I would do X" symptom), while an over-permissioned `build` on a read-only command is harmless, so `plan` is assigned only on a high-confidence read-only signal. A `plan` classification that's wrong for a specific command is fixed by adding an explicit `agent:` to the source (rule 1); a hand-authored `agent:` in the native `.opencode` file is also preserved across re-syncs.

**Token interpolation in command body**:
- `$ARGUMENTS` — full raw argument string
- `$1`, `$2`, ... — positional args (OpenCode parses on whitespace by default)
- `!`shell-cmd`` — shell injection (user confirms before execution)
- `@file` — inlines file contents at render time

User invokes: `/endpoint-test <controller>` in OpenCode chat.

Path: `.opencode/commands/<name>.md` (no prefix — filename = slash-command name).

> **Backward-compat fallback**: the same command can ALSO be mirrored into `opencode.json` `commands.<name> = {description, template}` for older OpenCode builds that pre-date `.opencode/commands/`. Mirror only on `--legacy-opencode` flag; not the default.

### Agents — `.claude/agents/<name>.md` → `.opencode/agents/<name>.md` (NATIVE)

OpenCode now reads `.opencode/agents/<name>.md` as a first-class persona definition:

```markdown
---
name: backend-reviewer
description: Reviews backend code for repository / service / controller hygiene
mode: subagent
model: anthropic/claude-sonnet-5
tools:
  read: true
  edit: false
  bash: false
---

<agent body from .claude/agents/backend-reviewer.md>
```

OpenCode picks this up the same way Claude Code picks up `.claude/agents/<name>.md`. Manual invocation in chat: `@backend-reviewer review this PR diff` (OpenCode's persona-mention syntax).

**Frontmatter is NOT a 1:1 copy — three transforms are mandatory** (a verbatim copy of the `.claude` agent loads broken or off-contract):
1. `tools:` — Claude's comma-string (`tools: Read, Grep, Edit`) → YAML record (`tools:\n  read: true`). OpenCode rejects the string form outright (`expected record, received string`).
2. `mode: subagent` — inject when absent, so OpenCode routes the file as a dispatchable persona rather than a primary agent.
3. `model:` — Claude tier shorthand (`sonnet`/`opus`/`haiku`) → provider id (`anthropic/claude-sonnet-5` …); a bare alias does not resolve in OpenCode's catalogue.

These are applied **deterministically** by `apply-adapter-sync.sh`'s `opencode_normalize_agent_frontmatter` (idempotent), and **enforced** by `audit-adapter-coverage.sh`'s `opencode_agent_frontmatter_ok`, which fails coverage for any agent that does not satisfy all three. Do not hand-author agent files that skip them — that is exactly how a botched sync ships undetected.

`AGENTS.md` still gets a `## Named personas` section as documentation for users who don't know the `@` syntax — but the actual persona behavior is driven by `.opencode/agents/<name>.md`.

### Skills — `.claude/skills/<name>/` → `.opencode/skills/<name>/` (NATIVE folder copy)

OpenCode reads `.opencode/skills/<name>/SKILL.md` per-project natively (same shape as Claude). The adapter copies the entire skill folder verbatim:

```
.opencode/skills/module-scaffold/
├── SKILL.md            # Frontmatter + body, copied 1:1 from .claude/skills/module-scaffold/SKILL.md
├── scaffold.sh         # Copied verbatim
└── templates/          # Sub-resources copied verbatim
```

`SKILL.md` frontmatter:

```markdown
---
name: module-scaffold
description: Scaffold a new NestJS module matching project conventions
---

<skill body>
```

OpenCode surfaces this in its skills picker. Supporting scripts run via OpenCode's bash tool. **No symlink to `~/.claude/skills/` required** — the project is self-contained.

### Hooks — `.claude/hooks/*.sh` → `.opencode/plugins/*.ts` (NATIVE — plugin event handlers)

> **OpenCode hooks correction (2026-07)**: prior versions of this adapter claimed OpenCode had "no lifecycle hooks." **That is stale.** OpenCode ships a plugin system whose event handlers cover the guardrail surface Claude Code hooks provide — `tool.execute.before` can block or rewrite a tool call, `tool.execute.after` fires post-execution, and there is a rich session/file/permission event set. The repo's `.claude/hooks/*.sh` become OpenCode **plugins**; git hooks are now only a residual fallback.

**Native support:** partial ~ — full lifecycle event coverage and pre-execution blocking, but plugins are **JS/TS modules**, not the `settings.json` + shell-command declaration Claude/Codex/Gemini share. The repo's `.sh` scripts must be wrapped by a thin plugin that shells out to them, rather than declared directly.

**Native mechanism:** `.opencode/plugins/*.{js,ts}` (project) or `~/.config/opencode/plugins/` (global), auto-loaded at startup; npm plugins listed in `opencode.json`'s `"plugin"` array. A plugin exports a function receiving `{ project, directory, worktree, client, $ }` (`$` is Bun's shell API) and registers async event handlers. Events: **`tool.execute.before`**, **`tool.execute.after`**, **`session.created`/`.idle`/`.compacted`/`.updated`/`.deleted`**, **`file.edited`**, **`file.watcher.updated`**, **`message.updated`/`.part.updated`**, **`permission.asked`/`.replied`**, **`command.executed`**, **`shell.env`**.

**Translation of the repo's hooks** (`templates/repo-baseline/.claude/hooks/`):

| Claude Code hook | OpenCode native event | Wiring |
|---|---|---|
| `guard-destructive.sh` + `pre-edit-guard.sh` + `secret-scan.sh` | **`tool.execute.before`** | `throw` inside the handler to block the tool call (e.g. destructive bash, protected-path edit, secret leak); can also mutate args to rewrite |
| `module-boundaries.sh` | **`tool.execute.before`** | `throw` when the incoming edit adds an import that crosses a boundary declared in `ai/modules.md`; the plugin must pass the edit **body** through to the script — it inspects the pending content, not the file on disk |
| `post-edit-check.sh` + `format-on-save.sh` + `auto-test.sh` | **`tool.execute.after`** (or **`file.edited`** for format-on-save) | run lint/format/test after an edit tool completes |
| `inject-path-rules.sh` | no path-scoped-rules primitive → **`instructions` globs** in `opencode.json` (always-apply) **or** a `chat.message` handler injecting context | OpenCode loads all `instructions` markdown as context; path-conditional injection needs a plugin |
| `inject-blast-radius.sh` | no equivalent in `instructions` globs → a **`tool.execute.before` handler** that shells out to the hook and injects its output | The `instructions` route works for `inject-path-rules.sh` because those are static rules; this payload is per file and computed, so only the handler route carries it. |
| `notify.sh` | **`session.idle`** | fire a notification when the turn goes idle |
| `session-start.sh` | **`session.created`** | surface `ai/status.md` / phase at session open |
| `update-session-log.sh` | **`session.idle`** | append to `ai/dynamic/session-log.md` |
| `verify-gate.sh` | **`session.idle`** | fires at turn end, but no documented way to refuse the finish — `throw` is specified for `tool.execute.before`. Report there and keep `.husky/pre-commit` as the gate rather than claiming a blocked stop |

```typescript
// .opencode/plugins/guardrails.ts
export const guardrails = async ({ $ }) => ({
  "tool.execute.before": async (input, output) => {
    // input.tool, output.args — shell out to the repo's existing script
    const res = await $`.claude/hooks/pre-edit-guard.sh`.quiet().nothrow()
    if (res.exitCode !== 0) throw new Error(res.stderr.toString())  // blocks the tool call
  },
  "tool.execute.after": async (input, output) => {
    await $`.claude/hooks/post-edit-check.sh`.nothrow()
  },
})
```

**Fallback residue** — only if a project opts out of plugins: keep `.husky/` (or vanilla `.git/hooks/`) replicating `post-edit-check.sh` + `pre-edit-guard.sh`, plus the `AGENTS.md` "Before starting a session / Before committing" disclosure. Flag non-enforced hooks in `ai/references/tool-parity.md`.

**Caveats:**
- **JS/TS wrapping required** — unlike Codex/Gemini (shell-command hooks in a config file), OpenCode plugins are code; the repo's `.sh` scripts run only via a `$`-shell wrapper inside a plugin. The event payload (`input.tool`, `output.args`, `output.result`) is JS objects, not the stdin JSON schema the scripts expect — the wrapper bridges them.
- **Evolving surface** — several events are marked experimental (`experimental.chat.messages.transform`, `experimental.session.compacting`); pin behavior against https://opencode.ai/docs/plugins/ before relying on them. Local project plugins load **last** in order, so they can override global ones.
- Plugins execute in-process; a throwing/slow handler stalls the agent.

**Source:** https://opencode.ai/docs/plugins/

## Cross-references

- **Migration pack — companion scripts (2026-05):** OpenCode has no hooks — run the **full** script bundle from the shell / CI (`validate-migration-artifacts.sh`, `migration-doctor.sh`, `migration-reachability.sh`, etc.). Canonical list: `templates/tool-adapters/_migration-pack-coverage.md` § **Companion scripts (2026-05)**.
- **Optimize pack — companion scripts (2026-05):** Run **`validate-optimize-artifacts.sh`** + **`optimize-parallel.sh`** from shell/CI; see `templates/tool-adapters/_optimize-pack-coverage.md`.
- **Refactor pack — companion scripts (2026-05):** Run **`validate-refactor-artifacts.sh`** for `/refactor` ledger rows; see `templates/tool-adapters/_refactor-pack-coverage.md`.
- **Polish pack — companion scripts (2026-05):** Run **`validate-polish-artifacts.sh`** + **`polish-parallel.sh`** from shell/CI for `/polish` (stack-conditional — frontend / backend / data / mobile evidence); frontend rows additionally gated by **`check_frontend_verb_vocabulary`** against the closed 19-verb **`ui-design-sweep`** set (ui-ux pack v1.1+); see `templates/tool-adapters/_polish-pack-coverage.md` + `templates/tool-adapters/_ui-ux-pack-coverage.md`.
- **Align pack — companion scripts (2026-05):** Run **`validate-align-artifacts.sh`** + **`align-parallel.sh`** from shell/CI for `/align`; see `templates/tool-adapters/_align-pack-coverage.md`.
- **Audit pack — companion scripts (2026-05):** `/audit` artifacts live under `ai/audit/**` (`plan.md`, `progress.md`, per-axis subfiles `_arch.md` / `_quality.md` / `_security.md` / `_db.md` / `_perf.md` / `_scale.md` / `_infra.md` / `_obs.md`, `final-report.md`, `assessment.md`). **Three output modes**: default (execute), `--plan-only` (ranked fix-plan in `plan.md` — executor handoff), `--assess` (8-section senior-engineer narrative in `assessment.md` — what's good / improve / unify / extract / simplify / redesign / remove / optimize — reader handoff). Validator **`validate-audit-artifacts.sh`** ships in `scripts/` — halts on hand-waves (`etc.`, `would be slow`, `at scale this is bad`) and requires P0 findings to cite a target-RPS failure mode (`check_no_handwaves_audit_plan` + `check_p0_failure_mode_cited`; `--strict` adds `<file:line>` citation enforcement on P0/P1/P2). Dispatches existing pack scripts internally (`validate-optimize-artifacts.sh`, `validate-align-artifacts.sh`, `validate-polish-artifacts.sh`) for the architecture / SOLID / clean-code / API-consistency axes; security + DB + scale axes use their respective pack agents and skills directly. See `commands/audit.md` + `templates/tool-adapters/_orchestration-sync.md`.
- **Unify-surfaces pack — companion scripts (2026-05):** `/unify-surfaces` (frontend-only, sibling to `/polish`) artifacts live under `ai/unify-surfaces/**` (`progress.md`, per-category inventory, canonical-wrapper decision evidence, `final-report.md`). Validator **`validate-unify-surfaces-artifacts.sh`** is **planned** — will check per-category inventory completeness, canonical-wrapper decision citations, idioms-update co-commit (`_extracted-idioms.md § Wrappers` updated in same commit), `Reuse-Before-Create` enforcement (extracting a duplicate where a shared wrapper exists fails). 7 default categories: tables / forms / headers / tabs / filters / buttons / validation. Validation is a 3-part pipeline (composable + components + API-error mapper), not a single wrapper. Halts on `PROJECT_KIND` not in `frontend-* / mobile-web / mobile-rn` with redirect to `/polish`. See `commands/unify-surfaces.md` + `templates/tool-adapters/_orchestration-sync.md`.
- **`/upgrade-dep` (dependency / framework / runtime upgrade, any ecosystem, 2026-09):** one bounded version span under ONE app — not a V1→V2 port (`/migrate`) and not a staleness report (`/dependency-vuln-check`). A translation MUST preserve four things or it is not this command: (1) **cite-or-halt on the vendor's breaking-change source** for the exact span — a major upgraded from recollection is the defect the halt exists for; (2) the **parity oracle is pinned and its baseline captured BEFORE the first edit**, chosen by `PROJECT_KIND` (suite + boot + route inventory + contract diff for `backend-*`; typecheck + build + route-mount sweep for `frontend-*` / `mobile-web`; **both** platform targets building for `mobile-rn` / `mobile-native` (Flutter and the native toolchains alike); exported-symbol diff for a library) — a project with no oracle gets the cheapest one committed first, never a silent pass; (3) **staged commits, never mixed** — manifest+lock, then the codemod's output verbatim and alone, then one commit per breaking-change item, then config re-shaping, so `git revert <codemod-sha>` stays a real rollback; (4) **majors one at a time** and, for a runtime-class upgrade, every in-repo pin (version file, manifest constraint, Dockerfile `FROM`, CI matrix, deploy runtime) moves in the same run or the run halts — a pin outside the repo is named under `Risks:`, never dropped. No ledger, no `progress.md`: it is one-shot. The run closes with the mandatory `Not validated:` / `Risks:` / `Revert:` block plus paste-ready next steps; **no `validate-*-artifacts.sh` ships for it** — the contract is carried by the command's own halt conditions, so an adapter must not claim a script gate it does not have. Surface it through the same native path this adapter already uses for `/migrate`. The run is shell-heavy (package manager, codemod, oracle); where this tool gates shell execution, document that seam rather than reporting an upgrade that never ran. See `commands/upgrade-dep.md` + `templates/tool-adapters/_orchestration-sync.md`.
- **Orchestration / validator sync:** **`templates/tool-adapters/_orchestration-sync.md`** — discipline paths (`ai/migrate/progress.md`), optimize oracle fallbacks, align 21-verb closure vocabulary, polish validator env (`QUIET=1`; no `--strict` CLI), refactor hook paths.
- **`/task` integration (MCP-backed):** provider-agnostic task executor — Trello / Jira / Linear / GitHub Issue → fetch → normalize → dispatch → write-back (in-progress → comment → review/done). This tool surfaces it as `.opencode/commands/task.md` (`agent: build`); needs a task-provider MCP from `scripts/detect-mcp.sh` (gated on the repo's own `.env` creds). It routes to OpenCode specialist commands in place of `/do`. Canonical recipe + per-tool matrix: `templates/tool-adapters/_task-integration-coverage.md`; lifecycle: `commands/task.md`; providers: `templates/integrations/task-providers.md`.
- **`/delegate` (cross-tool dispatch, CLI-backed):** hand ONE bounded task to a **different** AI coding CLI running as a separate process against this tree, then review the diff and commit it yourself — the relay never commits. This tool surfaces it as `.opencode/commands/delegate.md` with **`agent: build` REQUIRED** — Plan mode has read tools without write or shell, and `opencode_classify_command_agent` classifies deterministically, so `/delegate` must land on `build` in that mapping or it routes wrong and answers "I would run the relay". As an **implementer** it is `opencode` and it **requires `--model=`**: `P_NEEDS_MODEL=1` makes a bare `--to=opencode` hard-fail before launch (relay exit `2`), which is a precondition to state up front, not a parenthetical — the translated command must expose `--model=` and forward it, and must also rename `--to=` to the relay's `--implementer=`. Read-only `--agent plan`; resume relay-unsupported. Canonical recipe + per-tool matrix: `templates/tool-adapters/_delegate-integration-coverage.md`; contract: `commands/delegate.md`; relay: `scripts/delegate-relay.sh`.
- **Project-memory recall (`/recall` + `recall-inject.sh`):** BM25 retrieval over this project's existing `ai/` memory — the `ai/dynamic/` sinks, the don't-retry catalog at `ai/failures/_index.md`, promoted ADRs / patterns / conventions, and the archives. Adds no sink and stores nothing; every row is a **POINTER** (`path:line`), never a paraphrase. Two halves that rank tools differently: the `/recall` command a human invokes, and the opt-in (`touch .claude/.recall`) prompt-time hook that surfaces the same memory unasked. **Query yes, injection UNKNOWN.** `.opencode/commands/recall.md` with **`agent: build`** runs the engine (plan mode has read tools but no shell); add `recall` to `opencode_classify_command_agent` or it defaults to the wrong agent. For the hook half, this adapter names `tool.execute.before` / `tool.execute.after` and a session/file/permission event set — **no prompt or chat-message event appears anywhere in it**, so no claim is made. What would settle it: the plugin event list naming a prompt-submit event *and* whether its handler return value is appended to context. Note that even a confirmed event needs more work here than elsewhere: plugins are TS, so the `.sh` runs only through a `$`-shell wrapper with a JS-object→stdin JSON bridge. Canonical recipe + per-tool matrix: `templates/tool-adapters/_memory-recall-coverage.md`; command: `templates/packs/learning/commands/recall.md`; hook: `templates/repo-baseline/.claude/hooks/recall-inject.sh`; engine: `scripts/pack-search.py --catalog=memory` + `scripts/gen-memory-catalog.py` (**install the pair into `~/.claude/scripts/` — same directory, or the hook silently no-ops**).
- **Actionable next steps — universal report contract (2026-05):** Every report-producing command (`/optimize`, `/polish`, `/align`, `/migrate`, `/refactor`, `/audit`, `/unify-surfaces`) MUST end its `final-report.md` with a `## Actionable next steps` section per **`templates/snippets/actionable-next-steps.md`** — paste-ready commands. Validator gate: **`check_actionable_next_steps`** halts when missing or prose-not-args. Run from shell/CI on edits under `ai/{optimize,polish,align,migration,refactor}/**`.
- **Agent frontmatter normalization + enforcement (2026-06):** A 1:1 copy of `.claude/agents/<name>.md` loads broken in OpenCode. `apply-adapter-sync.sh`'s **`opencode_normalize_agent_frontmatter`** (renamed from `opencode_normalize_agent_tools`) now applies **three** deterministic, idempotent transforms — string `tools:` → YAML record, inject `mode: subagent`, map `model:` tier shorthand (`sonnet`/`opus`/`haiku`) → `anthropic/claude-…` provider id. **`audit-adapter-coverage.sh`**'s new **`opencode_agent_frontmatter_ok`** gates coverage: an agent that merely exists but is off-contract no longer scores as a hit (closes the silent-pass gap where a botched `/setup-project-adapters` run shipped string-`tools:` agents undetected). See § Agents above.

- `codex/adapter.md` — AGENTS.md format lives there.
- `claude-code/adapter.md` — source of rules content.
