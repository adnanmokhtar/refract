...
| Hooks (lifecycle shell scripts) | `.claude/hooks/*.sh` | Claude Code, Cursor (`.cursor/hooks.json`), Codex (`.codex/hooks.json`), Gemini (`.gemini/settings.json`), Qwen (`.qwen/settings.json`), Copilot (`.github/hooks/*.json`), Cline (`.clinerules/hooks/<Event>`), Windsurf (`.windsurf/hooks.json`), Kimi (`~/.kimi/config.toml`, user-level); OpenCode ~ (`.opencode/plugins/*.ts`), Aider ~ (`lint-cmd`/`test-cmd`, post-only) | Continue only (no agent-loop hooks) → Husky git hooks + always-apply rules + `ai/status.md` briefing. Every native target needs a thin per-tool payload shim (differing event JSON, block fields, timeout units). |
...
- [`_memory-recall-coverage.md`](_memory-recall-coverage.md) — **cross-cutting (spans two primitive classes):** project-memory retrieval over the `ai/` tree. The `/recall` command a human invokes **and** the opt-in `recall-inject.sh` prompt-time hook, which rank tools *differently* — Cline's injection half is native while its shell surface is unproven; Copilot / Windsurf / Gemini run the query half and have no injection surface at all. Carries the three different output envelopes, the payload shim, and the `pack-search.py` + `gen-memory-catalog.py` pair-install obligation (a half-install is silent, not an error).
