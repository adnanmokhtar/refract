...
- `module-boundaries.sh` — blocks an edit whose new import crosses a boundary declared in `ai/modules.md` (`A MUST NOT import from B`, `A MAY import from B only via <facade>`). Reads the incoming edit; TS/JS, Vue/Svelte, Python and Dart; silent when no boundary is declared.
...
- **Template-pack authoring:** When editing commands/snippets in the Refract repo’s `templates/`, run Phase 5 checks documented in `templates/tool-adapters/_template-author-scripts.md` (`audit-stack-leakage.sh`, `audit-command-dry.sh`; canonical pointers `templates/governance/core-discipline.md`, `templates/snippets/`).
- **Migration pack — companion scripts (2026-05):** With `--include=migration`, install the **full** Refract `scripts/` bundle into `~/.claude/scripts/` (validator + `migration-doctor.sh` + `migration-reachability.sh` + `migration-detect-existing.sh` + `migrate-parallel.sh` / `parallel-fan-out.sh`), not only `validate-migration-artifacts.sh`. Canonical list: `templates/tool-adapters/_migration-pack-coverage.md` § **Companion scripts (2026-05)**.
...
- **`/delegate` (cross-tool dispatch, CLI-backed):** hand ONE bounded task to a **different** AI coding CLI running as a separate process against this tree, then review the diff and commit it yourself — the relay never commits. This tool surfaces it as `.claude/commands/delegate.md` (native slash command) and is the source of truth — `/delegate` ships here; `scripts/delegate-relay.sh` must be installed into `~/.claude/scripts/` alongside the validators, or the translated command is a document rather than a command. As an **implementer** it is `claude` (brief on stdin, read-only `--permission-mode plan`, resume `--resume`) — but dispatching to `claude` from Claude Code is the **self-dispatch no-op**: same vendor, same model family, same blind spots, so pick a different implementer. Read-only is a tri-state tripwire (`enforced` / `unverified` / `unenforceable`, with `readOnly.violation` free to stay `null`), never a guarantee — rounding it to "read-only ✓" is the one failure the command exists to prevent. Canonical recipe + per-tool matrix: `templates/tool-adapters/_delegate-integration-coverage.md`; contract: `commands/delegate.md`; relay: `scripts/delegate-relay.sh`.
- **Project-memory recall (`/recall` + `recall-inject.sh`):** BM25 retrieval over this project's existing `ai/` memory — the `ai/dynamic/` sinks, the don't-retry catalog at `ai/failures/_index.md`, promoted ADRs / patterns / conventions, and the archives. Adds no sink and stores nothing; every row is a **POINTER** (`path:line`), never a paraphrase. Two halves that rank tools differently: the `/recall` command a human invokes, and the opt-in (`touch .claude/.recall`) prompt-time hook that surfaces the same memory unasked. **Source of truth for both halves.** `.claude/commands/recall.md` (learning pack) runs the engine; `.claude/hooks/recall-inject.sh` is wired to `UserPromptSubmit` in `.claude/settings.json` and emits `hookSpecificOutput.additionalContext` — the envelope every other adapter's shim is measured against. Derived cache at `.claude/_memory-index.json` (gitignored by the Phase 4.0 block, never committed); per-session dedup keys on the payload's `session_id`. Canonical recipe + per-tool matrix: `templates/tool-adapters/_memory-recall-coverage.md`; command: `templates/packs/learning/commands/recall.md`; hook: `templates/repo-baseline/.claude/hooks/recall-inject.sh`; engine: `scripts/pack-search.py --catalog=memory` + `scripts/gen-memory-catalog.py` (**install the pair into `~/.claude/scripts/` — same directory, or the hook silently no-ops**).
