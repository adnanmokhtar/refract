...
| **Cross-tool dispatch** (translatable, **CLI-backed**) | `/delegate` | **Translated to every adapter whose command primitive can run shell** — the relay (`scripts/delegate-relay.sh`) is tool-agnostic bash, so the **dispatcher** side ports everywhere the *Command translation* verdict above says "✅ Executes" AND the primitive reaches a shell. The **implementer** side is decided by `command -v` over the relay's own 10-name list, NOT by this matrix: `continue` and `windsurf` have adapters but no headless CLI and cannot be implementers, while `cursor-agent` and `cline` ship headless binaries and can. Aider has no executable primitive at all and degrades to an EXECUTE-NOW preamble that has the **human** run the relay from a terminal; Continue can do neither role — `continue/adapter.md` records that it does not execute shell from a prompt, and the honest entry is a NOT-SUPPORTED note. Per-tool dispatcher primitive, implementer binary, mandatory frontmatter field, and the read-only tri-state live in **`_delegate-integration-coverage.md`**. |
...
Adapters with no headless mode (Cursor, Cline, Windsurf — IDE-bound) cannot be drivers for parallel orchestrators. **Scope note:** that is true of the *editor* integrations and of `_parallel-tool-config.sh`'s driver set — it is **not** a claim about the vendors' CLIs. `cursor-agent` and `cline` ship headless binaries and are valid `/delegate` implementers (`commands/delegate.md` § Implementer matrix); `windsurf` ships none and is not one. Their users either run the underlying pack commands serially OR switch tools for the whole-project sweep step.
...
## Maintaining pack templates (Refract repo)
