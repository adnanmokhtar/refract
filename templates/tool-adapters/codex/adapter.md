...
> **Agent Skills correction (2026-05; first sourced 2026-08-21)**: prior versions of this adapter claimed Codex had "no executable primitive." That was wrong. Codex builds on the **open agent skills standard** (agentskills.io) — "A skill is a directory with a `SKILL.md` file plus optional scripts and references." Discovery precedence is `$CWD/.agents/skills` → `$CWD/../.agents/skills` → `$REPO_ROOT/.agents/skills` → `$HOME/.agents/skills` (user) → `/etc/codex/skills` (admin) → skills bundled with Codex. Skills are invoked by `$`-mention in the Codex CLI, or auto-selected when the description matches. Action commands like `/optimize` translate to **Agent Skills**, not to imperative-prose-only AGENTS.md sections. The AGENTS.md fallback remains for tools that consume `AGENTS.md` but lack the skills standard.
>
> **Source** (this paragraph only): <https://developers.openai.com/codex/skills>, which 308-redirects to <https://learn.chatgpt.com/docs/build-skills>; fetched 2026-08-21. It was added to `_version.json` `docs_urls` because **neither original URL supports this claim** — `https://agents.md` and the AGENTS.md guide both cover `AGENTS.md` only and mention no skills standard, so until now the claim rested on nothing fetchable. `last_verified` (2026-04-28) covers the AGENTS.md URLs and was deliberately **not** bumped: only the skills claim was re-checked here, not this adapter's full coverage, and the new URL carries no recorded hash yet (`check-tool-versions.py` will warn until `--update` records one).
...
- **Migration pack — companion scripts (2026-05):** Document in `AGENTS.md` / setup notes that migration users install the **full** script bundle from Refract's `scripts/` into `~/.claude/scripts/`, not only `validate-migration-artifacts.sh`. Canonical list: `templates/tool-adapters/_migration-pack-coverage.md` § **Companion scripts (2026-05)**.
...
- **`/delegate` (cross-tool dispatch, CLI-backed):** hand ONE bounded task to a **different** AI coding CLI running as a separate process against this tree, then review the diff and commit it yourself — the relay never commits. This tool surfaces it as `.agents/skills/delegate/SKILL.md` (Open Agent Skills — Codex has no user-extensible slash surface, so it is invoked via `/skills` or `$delegate`). Hosting is **partial**, and honestly so: Codex has a Bash tool and can launch the relay, but this adapter also records "no headless mode for fully autonomous loops — the user must press through approval prompts", which turns the 30-minute watchdog into a human-attended wait. **That line contradicts the bullet directly above it** (parallel workers spawned per row via `codex exec`) and `_registry.md`'s verified `codex exec`; until one of them is corrected, treat hosting as partial rather than pick a side. As an **implementer** it is `codex` (read-only `--sandbox read-only`, resume `codex exec resume <id>`). Canonical recipe + per-tool matrix: `templates/tool-adapters/_delegate-integration-coverage.md`; contract: `commands/delegate.md`; relay: `scripts/delegate-relay.sh`.
- **Project-memory recall (`/recall` + `recall-inject.sh`):** BM25 retrieval over this project's existing `ai/` memory — the `ai/dynamic/` sinks, the don't-retry catalog at `ai/failures/_index.md`, promoted ADRs / patterns / conventions, and the archives. Adds no sink and stores nothing; every row is a **POINTER** (`path:line`), never a paraphrase. Two halves that rank tools differently: the `/recall` command a human invokes, and the opt-in (`touch .claude/.recall`) prompt-time hook that surfaces the same memory unasked. **The cheapest port in the set — the script runs unmodified.** Codex's `UserPromptSubmit` event name *and* its `hookSpecificOutput.additionalContext` output envelope match Claude Code exactly, so `.codex/hooks.json` needs no payload shim; recall is the **second tenant** on the event this adapter already uses for `inject-path-rules.sh`, not a new mechanism. Query half is `.agents/skills/recall/SKILL.md` — reached via `/skills` or `$recall`, not a slash command. Canonical recipe + per-tool matrix: `templates/tool-adapters/_memory-recall-coverage.md`; command: `templates/packs/learning/commands/recall.md`; hook: `templates/repo-baseline/.claude/hooks/recall-inject.sh`; engine: `scripts/pack-search.py --catalog=memory` + `scripts/gen-memory-catalog.py` (**install the pair into `~/.claude/scripts/` — same directory, or the hook silently no-ops**).
