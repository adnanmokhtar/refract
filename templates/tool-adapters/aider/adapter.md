...
- `inject-blast-radius.sh` → **not event-driven, and unlike `inject-path-rules.sh` it cannot be folded into `CONVENTIONS.md`** — the payload is computed per file at edit time, not a fixed rule. Nearest port: a `pre-commit` step running `build-graph.py --who-breaks` over the staged paths, which warns after the edit rather than before it. Say so in `CONVENTIONS.md`: the edit is unguarded, the commit is not. **That leaves the pre-edit half uncovered, and it is the half that matters** — a warning after the commit is a warning after the decision. Cover it the way `continue` and `copilot` do: `AGENTS.md` § Discipline enforcement rule 9 instructs the agent to run `build-graph.py --who-breaks <file>` BEFORE changing a shared file, and `.aider.conf.yml`'s `read:` list already loads `AGENTS.md`. Manual, but pre-edit.
- `verify-gate.sh` (Stop) → **no Stop event.** The gate's job is to refuse "done" while the repo has uncommitted changes and a failing test command — Aider's `test-cmd` runs after each *edit*, not at turn end, so it cannot express "block the finish". Move it to `.husky/pre-commit` (fires on Aider's auto-commit, `--no-verify` is off) and say so in `CONVENTIONS.md`: the stop is unguarded, the commit is not.
