...
The pack provides twelve commands, five critical skills and the per-screen craft pair:
- **`/design-first <scope>` (v1.28+)** — the pack's **front door**: one command for "I want a design to look at", for anyone who does not already hold the ten-command map. Writes **nothing** — not code, not artboards, not tokens — it resolves the path and dispatches with the canvas gate on. Its whole substance is one question and one test, which look alike and are opposite in kind: `$SOURCE` (does this surface stay inside our design system or leave it) is a **permission** only the product owner holds and is **always asked**; the language-or-composition split is a **diagnosis** earned from a render and is **always run, never asked** — a run that asks it has failed at its one job. Six routing rows, every one ending on a canvas. Deliberately **not** named `/design`: Claude Code ships its own bundled `design` skill and the two must never contend for one invocation. Flags: `--source=`, `--no-canvas`, `--plan`. Frontend / mobile, plus project-optional `independent` routes.
...
- **`/ui-audit [<scope>]` (v1.30+)** — the pack's **autonomous** project-wide run and the visual sibling of the top-level `/audit`: scans every route from a real browser, cross-axis ranks every finding, **decides the design direction itself**, and executes V0→V3 to completion without a single question. It is not a new detector layer — it is the driver over `/ui-crawl` (browser truth) + `/ui-sweep`'s 8 detectors + `redesign.md`'s language-or-composition test + `creative-director`'s 9-lens Direction rubric + `ui-design-sweep`'s closed 19 verbs + `/redesign`'s composition rebuild. **Its substance for a porter is the autonomy contract, not new machinery**: each gate the other commands hold for a human (`$SOURCE`, the `/redesign` approval, the `/enhance-ui` variant pick, the `/art-direct` language call, `/ui-sweep`'s phase stop) is resolved mechanically and the mechanism is named, against a **five-item HALT list** (blocked render · no token source · dirty tree or red CI · a fix that re-detects · `--reimagine` on a multi-theme app). Writes `ai/ui-audit/{plan,progress}.md` + an HTML report. Frontend / mobile only.
...
- **`ui-designer` agent + `design-score` skill (v1.31+)** — the per-screen craft pair, and the answer to a run that reported every page clean without rendering one. `design-score` turns ONE render into ONE scorecard (per-lens + per-component + `at-bar`/`below-bar`) against `redesign.md`'s rubric — the single source, never restated — and carries two halts a porter MUST preserve: **no render, no score, ever**, and an anti-cheat that returns `RESCORE-REQUIRED` for a scorecard whose every verdict could have been written from metrics alone. `ui-designer` then takes a `below-bar` scorecard and proposes the change, naming the element, the existing token that delivers it, and the lens it moves; it is the only agent in the pack permitted to find a screen correct, conformant, usable and still not good enough. **Neither degrades to a source-only mode** — an adapter without a render harness routes to `/ui-sweep` and says so, rather than emitting a score nothing looked at.
...
- **`design-canvas` skill (v1.27+)** — drafts a surface / flow / feature as **artboards on one pan/zoom canvas** — a link a non-engineer opens, reacts to and annotates — **before any product code exists**. The only pack artifact whose output is not a diff, and the visual form of `/redesign`'s and `/art-direct`'s approval gate. Writes **only** under `ai/design/canvas/`, never product source. Asks exactly one question it refuses to infer — `$SOURCE` = **`system`** (lift the repo's resolved token + component literals and match exactly) vs **`independent`** (free of the repo: landing pages, greenfield) — which is a *permission* the product owner holds, deliberately unlike `/redesign`'s language-or-composition *diagnosis*, which is forbidden to ask. Draws the **states** (empty / loading / error / overflow), not the happy path alone, and halts on a happy-path-only set. **Two renderers, one authored fragment**: the **static** canvas (`canvas.html`, self-contained, opens in any browser) is the contract every tool gets; Claude Code's bundled WYSIWYG payload is an opportunistic upgrade that is **pinned to the CLI build and cannot be vendored**, so artboards are authored to the static renderer's floor — no holes, no tweaks, no logic class. An artboard needing the rich runtime is a defect. **Two floors (v1.29+)**: a **usability floor** computed and never asserted — contrast per role (4.5:1 body / 3:1 large / 3:1 non-text UI boundaries), target size 24×24 with the platform convention winning where stricter, not-by-colour-alone, 320px reflow — which is computable here precisely because every artboard value is a literal, with `NOT RUN` as a real printed state so a skipped check cannot read as a pass; and a **quality bar** citing `redesign.md § Design principles` rather than inventing a second vocabulary, splitting what a drawing can be scored on from what it cannot (motion is a build output; a drawing has no hover state) and naming the deferred lenses at every handover.
...
| Tool | `/ui-sweep` full | `/ui-crawl` + `/ui-crawl-fix` | `/redesign` (gate + rubric + render) | `ui-design-sweep` skill | `design-iterate` skill | Visual baseline (Playwright) | HTML report rendering | `design-canvas` skill |
|---|---|---|---|---|---|---|---|---|
| Claude Code | ✓ | ✓ (Playwright project at `tests/crawl/`) | ✓ (native command; gate pauses for approval) | ✓ (native skill) | ✓ (native skill) | ✓ (Playwright MCP) | ✓ (browser-side) | ✓ static + **editable** (bundled payload) |
| OpenCode | ✓ | ✓ (same Playwright project; commands at `.opencode/commands/`) | ✓ (`agent: build`; gate = explicit user-approval step) | ✓ (native skill) | ✓ (native skill) | ✓ (Playwright MCP) | ✓ | ✓ static canvas only |
| Cursor | ✓ | ✓ (commands at `.cursor/commands/`; runs against same Playwright project) | ✓ (Skill `redesign`; gate pauses for approval) | ✓ (native skill) | ✓ (native skill) | ✓ (Playwright MCP) | ✓ | ✓ static canvas only |
| Copilot | ✓ | ✓ prompt + Playwright runner via Actions | ✓ prompt; gate = approval checkpoint in prompt | ✓ (native skill) | ✓ (native skill) | conditional (if MCP wired) | conditional | ✓ static canvas only |
| Qwen | ✓ | ✓ command translation; Playwright project shared | ✓ (command; gate pauses for approval) | ✓ (native skill) | ✓ (native skill) | conditional | partial | ✓ static canvas only |
| Kimi | ✓ | ✓ skill wrapper; Playwright project shared | ✓ (skill wrapper; gate pauses for approval) | ✓ (native skill) | ✓ (native skill) | conditional | partial | ✓ static canvas only |
| Continue | ✓ rule + commands | as prompt template; relies on Playwright in repo | as prompt template; gate = approval checkpoint | as prompt template | as prompt template | conditional | partial | ✓ static canvas only |
| Cline / Roo | ✓ rule + workflows | inlined workflow; Playwright in repo | inlined workflow; gate = explicit `ask` step before edits | inlined in rule | inlined in rule | conditional | partial | ✓ static canvas only |
| Windsurf | ✓ rule + workflows | inlined workflow; Playwright in repo | inlined workflow; gate = approval step before edits | inlined in rule | inlined in rule | conditional | partial | ✓ static canvas only |
| Aider | rule-only | rule documents manual `pnpm crawl` invocation; no Playwright orchestration | rule-only (EXECUTE-NOW preamble); gate = "STOP, present proposal, await approval" | inlined in rule (closed-verb spec self-sufficient) | inlined in rule | rule documents manual procedure | NO (markdown summary instead) | ✓ static canvas only (rule-only authoring) |
| Codex | rule-only | rule documents manual invocation | rule-only; gate = explicit approval step in the procedure | inlined in rule (closed-verb spec self-sufficient) | inlined in rule | rule documents manual procedure | NO | ✓ static canvas only (rule-only authoring) |
| Gemini CLI | TOML command | rule documents manual invocation | `.gemini/commands/redesign.toml`; gate = approval step in `prompt` | inlined in rule (closed-verb spec self-sufficient) | inlined in rule | rule documents manual procedure | NO | ✓ static canvas only (rule-only authoring) |
...
**`/ui-audit` (v1.30+) combines the `/ui-sweep` and `/redesign` columns above and adds no new capability axis** — it needs exactly what they need (Playwright for the per-route renders, the HTML report surface, the 19-verb spec, `design-iterate` for Phase 3 candidates) and nothing else. What each adapter MUST translate instead is the **inversion**: where `/redesign`'s row says *"gate pauses for approval"*, `/ui-audit` says **"do not pause — commit per surface and let `git` be the approval"**, and a porter who copies the sibling's gate wording verbatim ships a command that stops on every surface and defeats its only reason to exist. The **HALT list is the part that must survive translation intact**: on rule-only tools (Aider / Codex / Gemini) the five HALTs become five explicit STOP conditions in the rule body, because a tool that cannot pause for approval must still pause for a blocked render — an unrendered route scored as clean is the one failure this command is built to refuse. Tools with no Playwright MCP **cannot run `/ui-audit` at all** (unlike `/ui-sweep`, which degrades to a source-only pass): every tier here verifies from a render, so the degraded mode would be a run that reports fixes it never saw. Those adapters document the requirement and route to `/ui-sweep` + `/polish` instead — a documented redirect, never a silent downgrade.
...
- `/ui-audit` → `.claude/commands/ui-audit.md` (reuses the Playwright MCP, the `creative-director` + `ui-designer` agents and the `design-score` / 19-verb skills; no new native surface)
