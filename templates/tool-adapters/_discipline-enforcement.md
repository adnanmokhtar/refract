...
This project follows the Refract pack discipline. Every tool reading this file — Claude Code, OpenCode, Cursor, Aider, Copilot, Cline, Windsurf, Codex, Gemini, Kimi — MUST honor the rules below for **every command**, regardless of how cleverly the tool's default behavior would otherwise route the work.
...
9. **Size the blast radius before changing a shared file.** `python3 ~/.claude/scripts/build-graph.py --corpus=project --repo=. --who-breaks <path>` answers "what breaks if I change this" from resolved imports, without opening a file — the total and the per-hop shape first (`74 at 1 hop · 203 at 2 hops · …`). `/setup-project` builds the cache (`run-preflight.sh` step 5/5); re-run the command above for a fresh answer after edits. It resolves TS/JS, Vue/Svelte, Python and Dart. **An empty answer means NO EDGE RESOLVED, never "safe to change"** — dynamic imports, DI containers and unresolvable build aliases produce no edge at all, so treat it as a floor on the blast radius and never as a clearance. Under a file-based router (Nuxt / Next / SvelteKit / Remix) a page or route file having no importers is CORRECT — the framework reaches it by path, not by import — and `--stats` says so when it applies. In Claude Code the same numbers arrive automatically via the `inject-blast-radius.sh` PreToolUse hook; every other tool ports that hook per its adapter's § Hooks, and where the tool has no per-edit hook this rule is the only route.
...
**Pack source hygiene (Refract repo only):** Editing commands under `templates/packs/` must stay DRY — link **`templates/governance/core-discipline.md`** and shared snippets instead of pasting SOLID glossaries; Phase 5 **`audit-command-dry.sh`** enforces this. See **`templates/tool-adapters/_template-author-scripts.md`**.
...
- `templates/tool-adapters/_template-author-scripts.md` — C2f/C2g scripts + canonical snippet pointers (Refract maintainers)
...
- `templates/packs/migration/rules/migration-discipline.md` — V1↔V2 paths, parity-test root and cutover mechanism live in its `project-specific` anchor block, injected by `apply-anchors.sh` from `ai/migration/_v2-anchors.md` (not a static section)
- `templates/packs/align/rules/align-discipline.md` — architecture / conventions / findings-ledger paths live in its `project-specific` anchor block, injected by `apply-anchors.sh` from the extraction sources (not a static section)
- `templates/packs/code-quality/rules/quality-principles.md` § Must not
