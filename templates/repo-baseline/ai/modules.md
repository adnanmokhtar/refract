...
- **It resolves relative specifiers and the `@/` `~/` `#/` sigils, and strips one leading `src/` / `app/` / `lib/`.** A build alias that RENAMES rather than shortens (`@core/*` → `src/billing/*`) IS resolved, by reading `compilerOptions.paths` from `tsconfig.json` / `jsconfig.json` (following `extends`). That table is what the compiler itself uses, so reading it is resolution rather than inference. With no config, no `python3`, unreadable JSON, or a prefix no rule declares, the alias table is empty and the hook stays silent rather than guessing — an alias can only turn *unresolvable* into *resolvable*, never change an answer the hook could already reach. `jest.moduleNameMapper` in `package.json` is read too, for the regex shapes that convert exactly. A project that aliases in webpack/vite config still loses them — those are JavaScript, reading them means executing it, and a guessed alias here would refuse a legitimate write; `rank-source-files.py` names such files on stderr rather than pretending they are not there.
