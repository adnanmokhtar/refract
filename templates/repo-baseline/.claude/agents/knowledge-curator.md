...
> **Installed copy.** This is the curator that ships into a project at `.claude/agents/knowledge-curator.md`. Its body is kept identical to the canonical source `templates/packs/learning/agents/knowledge-curator.md` (per that pack's `_topics.md` fallback). If you change one, change both — they must not diverge on which sinks they read.
...
- A proposed write would land outside a managed marker block → HALT — the curator NEVER writes outside `<!-- setup-project:managed -->` markers.
- A proposed update would contradict an accepted ADR without explicitly superseding it → HALT — cite the new ADR or stop.
...
- NOT auto-invoked: no shipped hook or cron runs this agent. A human dispatches it (see frontmatter `trigger:`).
