---
description: One command surface-type unification for frontend codebases. Deep multi-agent execution. Sibling to /polish — same simple-surface pattern, but typed by SURFACE CATEGORY (tables, forms, headers, tabs, filters, buttons, validation) instead of by axis. For each category: inventories every instance across the codebase, picks the canonical shape (extracts from `_extracted-idioms.md` or promotes the most-used pattern), creates / extends the shared wrapper, migrates every consumer, verifies parity. Form-validation gets a specialised pipeline — frontend validator composable + `<ErrorList>` / `<FieldError>` + API-error-to-field mapper as one extracted system. Frontend stacks only. NO phases visible, NO terminology, NO mid-run questions. Output is brief: surfaces unified, canonical wrappers, consumers migrated, commits, diff stats, test status. Distinct from /polish (axis-typed: tokens / rhythm / motion) and /align (drift-typed: any-class structural). /unify-surfaces is the multi-day whole-project simple-surface entry for "make the same surface look the same everywhere".
kind: command
pack: orchestration
---
...
- Per-axis polish across one surface type (tokens drift, motion drift, type-scale drift) → `/polish` (frontend).
