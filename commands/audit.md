---
description: One command, full-stack engineering audit — any language, framework, or project shape (backend / frontend / mobile / data / CLI / serverless / monorepo). Scans all axes in one pass: architecture, SOLID, clean code, security (OWASP / auth / tenant / secrets / deps), database + runtime performance, scalability + resilience (13 scale-lens detectors via PROJECT_KIND), infrastructure, observability. Ranks findings by impact-at-target-scale × blast-radius × fix-cost, then fixes in parallel waves. Modes: default (scan + rank + execute), --plan-only, --assess (senior-engineer narrative). The simple-surface alternative to running /optimize + security + db + perf audits separately.
kind: command
pack: orchestration
---
