...
Per the `canonical-command-template.md` flip ("commands declare which phases apply per type; skipping is the default"), phases are applied per project shape. `/scaffold-project` is by nature a CREATE-mode heavy command, so most phases run; but trivial scaffolds — single-file template injection, no architecture decisions — skip Phase 4.6/4.8 + Phase 7.

Default (non-trivial scaffold) — all 7 with **three confirmation gates**:
...
8. Phase 5 — Update (docs, ADRs, Claude orchestration) — ADRs land here, after Phase 6 boot test passes
9. Phase 6 — Validate (boots, lints, types)
10. Phase 7 — Improve (next-features roadmap)

Trivial scaffold (single-file template injection, no architecture decisions): skip Phase 4.6 (DB/ORM), Phase 4.8 (Claude orchestration layering), and Phase 7 (next-features roadmap). The skipped phases are logged in the first commit message so the audit trail is explicit.
...
Phase 4 generates code. Phase 5 documents choices (ADRs) once Phase 6's boot test passes. Writing ADRs before the scaffold even compiles is doc-before-code waste — we'd be locking in justifications for code that may not even build. Stage the choice list in memory here; defer the writes.

## Phase 5 — Update

Runs **after** Phase 6 confirms the scaffold boots. ADRs document choices that survived validation, not pre-validation hopes.
...
**Phase 6 mechanical halt**: If any check fails, refuse to advance to Phase 7. Print the failure list. Do NOT auto-continue. The user fixes or aborts. Phase 5 ADRs and Phase 7 next-features only run after every Phase 6 check is green.
